...
### 28.17 Exact API surface — *return/precondition types corrected by the Slice 2D-D1-A0 amendment; broadened by the third amendment (Blockers C/D/G); state-authority dependency added by the fourth amendment (Blockers B/C)*
...
recordExecutionPreCallFailure(session, deliberationState, store, liveExecutionPolicyResolver, input: { attemptId, failure: FailureInfo })
...
**`recordExecutionPreCallFailure` input (new, third amendment; policy-gated by the fourth):** `{ attemptId, failure: FailureInfo }` only. `route`, `deliberationStateId`, `claimedAt`, `terminalFactReadyAt`, `reservedLatencyMs` (`0`), and `actualLatencyMs` (`0`) are all derived — never caller-supplied; there is no caller-suppliable `authorized = true` flag of any kind. `failure.category` must be `REFERENCE_RESOLUTION` or `EXECUTION` (§28.33's restricted allowlist for this operation; `TRANSPORT`/`VALIDATION` are rejected outright, since neither a transport attempt nor an external response can exist before any call). Before recording any pre-call fact, the operation now resolves the same `liveExecutionPolicyResolver` as `claimRouteExecution` for the attempt's route (fourth amendment, Blocker C, §28.33) — a `DISABLED` result rejects/fails closed with **no checkpoint created**, exactly mirroring `claimRouteExecution`; only an `ENABLED` result permits a legitimate `REFERENCE_RESOLUTION`/`EXECUTION` pre-call fact to be recorded. See §28.33 for the complete precondition chain (no existing checkpoint, attempt resolves exactly, no existing `RouteOutcome`, route machine-executable, authoritative binding valid, live policy `ENABLED`) and for the causal precedence between a genuine `REFERENCE_RESOLUTION` failure and a merely-malformed policy-derived `latencyLimitMs`.
...
### 28.25 Required transition matrix — *updated by the Slice 2D-D1-A0 amendment; extended by the third amendment*
...
23. **EC-23** — *(added by the third amendment, Blocker G)* Checkpoint-backed `RouteOutcome` persistence (`persistExecutionTerminalFactAsRouteOutcome`) derives `status`/`latencyConsumed`/route-specific payload or `failure` exclusively from the stored terminal fact; a caller never re-supplies or retypes any of those facts (§28.35).
...
### 28.30 Recommended next slice — *title broadened by the third amendment; authorization gated by the fourth amendment's convergence-review requirement*

**Direction (unchanged): `SLICE 2D-D1-A` — EXECUTION COORDINATION / CLAIM / PRE-CALL FAILURE / LATENCY RESERVATION / COORDINATED LATENCY-WRITE GUARD OFFLINE RUNTIME**, exactly as Slice 2D-D1-0 already recommended (§27.22). The exact checkpoint entity, phase union (now including `INCONCLUSIVE` and the canonical pre-call-failure shape), API surface (now including the policy resolver, `DeliberationStateAccessPort`, `recordExecutionPreCallFailure`, and `persistExecutionTerminalFactAsRouteOutcome`), reservation-aggregation rule, unconditional cross-aggregate write-coordination rule, and transition matrix are now specified precisely enough to implement and test entirely offline (no provider calls), mirroring how `RouteAttempt`/`RouteOutcome` were themselves built and fully tested offline before any route's successful-result semantics existed. That runtime slice must implement, at minimum, the deterministic conformance tests anticipated in §28.31, including: (a) a two-concurrent-claims-same-`attemptId` test proving exactly one obtains a provider-call-authorized `ClaimedCheckpoint`, explicitly noting that a single-process in-memory test demonstrates API/store-contract conformance only, never cross-process production durability (§28.23); (b) a reservation-aggregation test proving two attempts against one `deliberationStateId` cannot jointly reserve more than the remaining ceiling, including a self-consistent-tamper variant where two individually-valid checkpoints' combined *effective* encumbrance exceeds the ceiling and global read integrity (§28.20) rejects it; (c) an admission-failure test proving the exact §28.8 transition is followed, with zero provider calls and no permanently-ambiguous `CLAIMED` state produced for a failure that was already known and pre-call; and the additional third- and fourth-amendment-specific tests listed in §28.31.

**Authorization status (fourth amendment): NOT GRANTED pending one final read-only convergence review.** This amendment fixes the third round of independently-demonstrated gaps in the same not-yet-implemented schema (the reservation/`RouteOutcome` handoff, the admission-rejection shape, and now claim-vs-write races, stale-state access, and the pre-call policy bypass). Per this packet's own instruction, committing this amendment does not itself authorize beginning `SLICE 2D-D1-A` — a separate, final, read-only convergence review (comparing every D1-0 `MUST`/required guarantee against §28's exact API surface, transition matrix, EC invariants, and every current latency-write surface) is required first, and only a review finding no implementation-blocking contract gap may authorize that runtime slice. A zero-finding review is evidence of convergence, not proof of correctness — it does not itself close the possibility of a future amendment.
...
**Caller input correction (resolves Blocker D concretely):** `claimRouteExecution`'s caller-facing input is `{ attemptId }` only (§28.18) — `latencyLimitMs` is never restated by the caller, closing both the missing-live-authorization gap and the caller-selected-latency-ceiling gap in one correction. Conceptually, the resolver and the store are fixed dependencies of the operation — acceptable as `RouteExecutionCoordinator { store, liveExecutionPolicyResolver }` with `coordinator.claimRouteExecution(session, deliberationState, { attemptId })`, or as a plain function `claimRouteExecution(session, deliberationState, store, liveExecutionPolicyResolver, { attemptId })` consistent with this document's established function-style API surface (§28.17) — **exact implementation class/function packaging remains an implementation choice; the authority relationship itself (resolver is composition-level, never per-call caller data) is not optional.**
...
### 28.33 Pre-call failure operation, category scope, and the route-readiness/corruption distinction (Slice 2D-D1-A0 third amendment, Blocker C; live-policy-gated by the fourth amendment)
...
**Preconditions** (all independently re-verified, never trusted from an earlier boundary, per this document's established "global before local" posture): no checkpoint already exists for `attemptId`; the attempt resolves exactly against the authoritative `deliberationState.attempts`; the authoritative session/state binding is valid (`verifyDeliberationBinding`); the attempt's `route` is machine-executable (§27.3 — `ADD_REVIEWER`/`REPLICATE`/`SEEK_EVIDENCE`/`TARGETED_PEER_CHALLENGE` only, never `ADD_CONTEXT`); no `RouteOutcome` already exists for `attemptId`; **the resolved `liveExecutionPolicyResolver` decision for this attempt's route is `ENABLED`** (fourth amendment, Blocker C, below); `failure.category` is legal for a pre-call failure (below). On success it derives `route`, `deliberationStateId`, `claimedAt`/`terminalFactReadyAt` (same instant), `reservedLatencyMs = 0`, and `actualLatencyMs = 0` — none of these are caller-supplied — clones the supplied `failure` snapshot (§28.22's alias-isolation rule), and enforces the same one-checkpoint-per-`attemptId` atomicity as `claimRouteExecution` (§28.10).
...
### 28.34 Cross-aggregate latency write coordination (Slice 2D-D1-A0 third amendment, Blocker B; hardened by the fourth amendment, Blockers A and B)
...
**28.34.4 State-level coordination critical section — now requires authoritative current-state access (fourth amendment, Blocker B):** freeze a `RouteExecutionCheckpointStore` capability conceptually:

```
withDeliberationStateCoordination(deliberationStateId, operation): result of operation
```

or an equivalent atomic/serialized mechanism, scoped per `deliberationStateId`, entered unconditionally (§28.34.3) by: (a) `claimRouteExecution`'s claim-and-reservation transaction (§28.7/§28.10); (b) `persistExecutionTerminalFactAsRouteOutcome` (§28.35); (c) `recordExecutionPreCallFailure`'s live-policy check where it touches shared state (§28.33); (d) **every** new latency-consuming write, checkpointed or not (§28.34.2/§28.34.3).
...
**Frozen wrapper for uncheckpointed writes, conceptually:**
...
Always enters state coordination first; resolves current state inside the boundary; evaluates current `spent` + current `effectiveLatencyEncumbrance` there; invokes the canonical product operation (`recordRouteOutcome`'s `ADD_CONTEXT` `SUPPLIED`/`DECLINED`/generic-`FAILED` branches, or `closeContextRequestWithoutResponse`'s `NO_RESPONSE` closure) only if legally admissible; commits its returned state before releasing. This covers every non-machine `ADD_CONTEXT` path without forcing `ADD_CONTEXT` into the provider-backed execution shell (§27.3/§28.4, unchanged).
