...
### 28.17 Exact API surface — *return/precondition types corrected by the Slice 2D-D1-A0 amendment; broadened by the third amendment (Blockers C/D/G)*
...
```
claimRouteExecution(session, deliberationState, store, liveExecutionPolicyResolver, input: { attemptId })
  → ClaimedCheckpoint | PreCallFailedTerminalFactReadyCheckpoint | throws
                                                        (§28.7/§28.8/§28.9/§28.32 — never throws for an ordinary
                                                         admission failure with a checkpoint-producing cause; DOES
                                                         throw/rejects with NO checkpoint created at all when the
                                                         resolved policy is DISABLED, §28.32 — never returns a
                                                         ReservedTerminalFactReadyCheckpoint. latencyLimitMs is no
                                                         longer caller input, third amendment — resolved internally
                                                         from liveExecutionPolicyResolver, §28.32)

recordExecutionPreCallFailure(session, deliberationState, store, input: { attemptId, failure: FailureInfo })
  → PreCallFailedTerminalFactReadyCheckpoint           (§28.33, third amendment, Blocker C — for pre-call failures
...
persistExecutionTerminalFactAsRouteOutcome(session, deliberationState, store, input: { attemptId })
  → DeliberationState                                  (§28.35, third amendment, Blocker G — coordination-layer
                                                         convenience operation: acquires state coordination,
                                                         derives RecordRouteOutcomeInput from the stored terminal
                                                         fact, calls the canonical recordRouteOutcome; never lets
                                                         a caller retype status/latency/payload/failure)
...
### 28.18 Claim input and terminal-fact input — narrowest legal shape — *terminal-fact input corrected by the Slice 2D-D1-A0 second amendment; claim input corrected by the third amendment*

**`claimRouteExecution` input:** `{ attemptId }` only — **`latencyLimitMs` is no longer part of the caller's input at all** (third amendment, Blocker D; see §28.32 for the full `LiveExecutionPolicyResolver` contract). Everything else — `route`, `deliberationStateId`, `sessionId`, hashes, `latencyLimitMs`, `reservedLatencyMs`, `claimedAt` — is derived from the authoritative `RouteAttempt` (re-resolved per §28.3), the live-execution policy resolver (a fixed dependency of the operation, never caller-supplied per call, §28.32), the deadline-violation claim-blocking check (§28.14), and the store's own transaction (§28.7). A caller may not restate any of it. A policy-derived malformed `latencyLimitMs` (§28.9 cause 1) is validated, never stored raw and never fabricated into a replacement value — it produces a `PreCallFailedTerminalFactReadyCheckpoint` with no `latencyLimitMs` field at all (§28.13). A `DISABLED` policy result is not a checkpoint-producing outcome at all (§28.32) — the operation rejects/fails closed with no execution-coordination record created.

**`recordExecutionPreCallFailure` input (new, third amendment):** `{ attemptId, failure: FailureInfo }` only. `route`, `deliberationStateId`, `claimedAt`, `terminalFactReadyAt`, `reservedLatencyMs` (`0`), and `actualLatencyMs` (`0`) are all derived — never caller-supplied. `failure.category` must be `REFERENCE_RESOLUTION` or `EXECUTION` (§28.20/§28.33's restricted allowlist for this operation; `TRANSPORT`/`VALIDATION` are rejected outright, since neither a transport attempt nor an external response can exist before any call). See §28.33 for the complete precondition chain (no existing checkpoint, attempt resolves exactly, no existing `RouteOutcome`, route machine-executable, authoritative binding valid).
...
### 28.19 Commit-acknowledgement input and integrity — narrowest legal shape, no adjacency-only trust — *tightened by the third amendment (Blocker G)*

**`markExecutionOutcomeCommitted` input:** `{ attemptId }` only, per this packet's own preference — a caller never simply asserts `outcomeCommitted = true`. The operation must independently verify, before transitioning:
...
|---|---|
| `claimRouteExecution` when the resolved live-execution policy is `DISABLED` (generally or for this route) | §28.32 — a pure reject/fail-closed with **no checkpoint created at all**, never a `PreCallFailedTerminalFactReadyCheckpoint`; "not authorized" is distinct in kind from "authorized but not admissible this time" |
...
| An uncoordinated latency-consuming write (any `applyLatencySpend` call site, §28.34) proceeding while positive active effective encumbrance exists for the same `deliberationStateId` | §28.34's cross-aggregate coordination rule (third amendment, Blocker B/EC-22) — the first implementation rejects/defers rather than risk `spent + effectiveLatencyEncumbrance > ceiling` |

### 28.26 Frozen design-time invariants (Slice 2D-D1-A0)

Numbered locally to this section (`EC-1`–`EC-25`) rather than merged into this document's master invariant numbering (§16 and elsewhere), because — consistent with Slice 2D-D1-0's own posture (§27.18–§27.19) — nothing in this section is implemented runtime; these are binding requirements a future runtime slice must satisfy, not yet-accepted guarantees about existing behavior.
...
22. **EC-22** — *(added by the third amendment, Blocker B)* Every new write that can increase `deliberationState.latencyBudget.spent` on a coordination-active `deliberationStateId` (every `applyLatencySpend` call site, §28.34's inventory — not only `recordRouteOutcome`) participates in the same state-level latency coordination boundary as claims and reservations; an active positive effective encumbrance cannot be bypassed by a second, uncoordinated product-write surface (§28.34).
...
### 28.33 Pre-call failure operation, category scope, and the route-readiness/corruption distinction (Slice 2D-D1-A0 third amendment, Blocker C)
...
### 28.34 Cross-aggregate latency write coordination (Slice 2D-D1-A0 third amendment, Blocker B)
...
**28.34.2 Full `applyLatencySpend` call-site inventory (source-grounded, this packet's own required inspection):** a search of every call site in `src/stress-test/deliberation.ts` (confirmed via `grep -na` — this file contains intentional literal `\x00` delimiter bytes elsewhere that make a plain-text `grep` silently treat it as binary and report zero matches; `-a` forces text mode) found **exactly eight**, all within two functions:

| # | Line | Function | Branch |
|---|---|---|---|
| 1 | 1889 | `recordRouteOutcome` | generic `FAILED`, any route |
| 2 | 1939 | `recordRouteOutcome` | `SEEK_EVIDENCE` success (`SUCCEEDED` or `INCONCLUSIVE`) |
| 3 | 2022 | `recordRouteOutcome` | `ADD_REVIEWER` success |
| 4 | 2083 | `recordRouteOutcome` | `ADD_CONTEXT` `SUPPLIED` success |
| 5 | 2106 | `recordRouteOutcome` | `ADD_CONTEXT` `DECLINED` success |
| 6 | 2152 | `recordRouteOutcome` | `TARGETED_PEER_CHALLENGE` success |
| 7 | 2192 | `recordRouteOutcome` | `REPLICATE` success |
| 8 | 2934 | `closeContextRequestWithoutResponse` | `ADD_CONTEXT` `NO_RESPONSE` dedicated closure (Slice 2D-C4) |

**Classification:** call sites 1–3, 6, 7 (`recordRouteOutcome`'s `FAILED`/`SEEK_EVIDENCE`/`ADD_REVIEWER`/`TARGETED_PEER_CHALLENGE`/`REPLICATE` branches) are the **checkpoint-backed future-machine path** — for these, coordination is achieved by routing every such write through `persistExecutionTerminalFactAsRouteOutcome` (§28.35), which itself acquires state coordination before calling `recordRouteOutcome`. Call sites 4–5 (`recordRouteOutcome`'s `ADD_CONTEXT` `SUPPLIED`/`DECLINED` branches) and call site 8 (`closeContextRequestWithoutResponse`) are **non-machine, dedicated-closure paths** — `ADD_CONTEXT` is categorically excluded from the provider-backed execution shell (§27.3/§28.4's scope), so these writes never go through a checkpoint at all, yet they consume latency from the **same** `deliberationState.latencyBudget` a coordination-active state's checkpoints are drawing against. This is exactly call site 8's role in the worked example above — confirming Blocker B is a real, demonstrated gap, not a hypothetical one. **No hidden ninth call site was found.**

**28.34.3 Frozen rule — one latency-write authority per coordination-active state:** `recordRouteOutcome` itself remains, unmodified, the canonical product-domain semantic validator (§28.27/EC-25 — not replaced, not redesigned). But for a **coordination-active** `deliberationStateId` — one currently carrying any checkpoint with positive `effectiveLatencyEncumbrance` (§28.11) — `recordRouteOutcome` (called directly, or `closeContextRequestWithoutResponse`, or any future latency-consuming operation) is no longer, by itself, a complete write-authorization boundary for latency concurrency. **Every** operation capable of increasing that state's `latencyBudget.spent` — every one of the eight call sites above, and any future one — must first enter the same state-level coordination boundary as claims and reservations (EC-22). This is a **write-authority/concurrency rule**, never a product-semantic one: it constrains *when* a write is admitted, not *whether* its content is valid, and it introduces no second product identity.

**28.34.4 State-level coordination critical section:** freeze a `RouteExecutionCheckpointStore` capability conceptually:

```
withDeliberationStateCoordination(deliberationStateId, operation): result of operation
```

or an equivalent atomic/serialized mechanism, scoped per `deliberationStateId`, used by: (a) `claimRouteExecution`'s claim-and-reservation transaction (§28.7/§28.10, unchanged); (b) `persistExecutionTerminalFactAsRouteOutcome` (§28.35); (c) any new latency-consuming write — checkpointed or not — attempted while that state carries positive active effective encumbrance. An in-memory, single-process implementation may satisfy this deterministically for offline tests — this proves API/store-contract conformance only, **never** production cross-process durability (§28.23, unchanged posture, restated for this new capability).

**28.34.5 Conservative first-implementation policy (correctness over concurrency):** if one or more checkpoints currently contribute positive active effective encumbrance for a `deliberationStateId`, an unrelated/uncheckpointed latency-consuming write for that same state is **not** permitted to bypass that reservation. The first runtime implementation MAY simply reject/defer such a write rather than attempt to share spare capacity concurrently — deliberately conservative, with no automatic retry implied. If no positive active encumbrance exists for the state, the canonical product-domain operation proceeds exactly as it does today, uncoordinated, unchanged.

**28.34.6 No retroactive checkpointing:** this rule never retroactively checkpoints a historical, already-recorded outcome or closure — it governs only **new** writes attempted while a state is coordination-active (§28.21's legacy posture, unchanged and restated: historical no-checkpoint `RouteOutcome`s remain legitimate forever).

**28.34.7 Claim-vs-write race, both interleavings must hold:** (A) a claim/reservation obtains the state coordination section first — the unrelated latency-consuming write, arriving second, sees the resulting authoritative `spent`/encumbrance and is rejected/deferred, never permitted to read a stale pre-claim value. (B) an uncoordinated latency-consuming write obtains the section first — the subsequent claim computes its own admission check (§27.12's `remainingLatency` formula) against the resulting, already-updated authoritative `spent` value, never a stale pre-write one. No interleaving may let both proceed against the same stale read.

### 28.35 Checkpoint-backed `RouteOutcome` persistence & exact terminal-fact/outcome agreement (Slice 2D-D1-A0 third amendment, Blockers G and E)
...
**Frozen sequence:** (1) acquire state-level coordination (§28.34.4) for the checkpoint's `deliberationStateId`; (2) resolve the exact `TERMINAL_FACT_READY` checkpoint for `attemptId`; (3) reject outright if `terminalFact.kind === 'DEADLINE_VIOLATION'` (§28.14 — never committable); (4) derive the exact `RecordRouteOutcomeInput` from the **stored** terminal fact — `status` from `terminalFact.kind` (§28.13's closed mapping: `SUCCEEDED → SUCCEEDED`, `INCONCLUSIVE → INCONCLUSIVE`, `FAILED → FAILED`), `latencyConsumed` from `terminalFact.actualLatencyMs`, and the route-specific fields spread in directly from `terminalFact.payload` or `terminalFact.failure` — a purely mechanical reassembly, since `RouteSpecificSuccessPayload`/`SeekEvidenceInconclusivePayload` were already frozen (§28.13) as exact subsets of `RecordRouteOutcomeInput`, never a new normalization step; (5) call the canonical, unmodified `recordRouteOutcome` with that derived input; (6) never call a provider at any point; (7) return the new `DeliberationState`; (8) if the product write fails or rejects, leave the checkpoint at `TERMINAL_FACT_READY` (§27.11's "product persistence failure" posture, unchanged) — the same checkpointed snapshot remains available for a later persistence-only reconciliation attempt (§27.11), never a second call to the coordination-layer's own terminal-fact-recording step. A caller supplies `{ attemptId }` only (§28.18) — never `status`, `latencyConsumed`, `failure`, `result`, `citations`, `targetRef`/`sourceRef`, `response`, `findingIds`, `boundedExcerpt`, or any other terminal-payload field; all are derived exclusively from the stored terminal fact (EC-23).
