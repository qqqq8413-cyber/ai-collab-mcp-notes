...
**Frozen wrapper for uncheckpointed writes, conceptually — checkpoint-exclusivity precondition added by the fifth amendment (Blocker B, see §28.36 for the full rule):**

```
performCoordinatedLatencyWrite(
  deliberationStateId,
  attemptId,
  operation: (currentState) => nextState
): DeliberationState
```

Always enters state coordination first; resolves current state inside the boundary; **checks `RouteExecutionCheckpointStore` for an existing checkpoint on `attemptId` — if one exists, rejects outright, regardless of that checkpoint's `reservedLatencyMs`/`effectiveLatencyEncumbrance` (§28.36)**; if none exists, evaluates current `spent` + current `effectiveLatencyEncumbrance` there and invokes the canonical product operation (`recordRouteOutcome`'s `ADD_CONTEXT` `SUPPLIED`/`DECLINED`/generic-`FAILED` branches, or `closeContextRequestWithoutResponse`'s `NO_RESPONSE` closure) only if legally admissible; commits its returned state before releasing. This covers every non-machine `ADD_CONTEXT` path without forcing `ADD_CONTEXT` into the provider-backed execution shell (§27.3/§28.4, unchanged) — and, per §28.36, the same checkpoint-existence check applies to any other coordinated attempt to persist a `RouteOutcome` directly, not only this specific wrapper.
