...
recordExecutionTerminalFact(store, input: { attemptId, actualLatencyMs, result: ExecutionTerminalResultInput })
  → ReservedTerminalFactReadyCheckpoint                (§28.18 — requires current phase CLAIMED; can never
                                                         ...
                                                         internally derives terminalFact.kind — SUCCEEDED, FAILED,
                                                         or DEADLINE_VIOLATION — from actualLatencyMs vs. the
                                                         checkpoint's own latencyLimitMs, never from caller input;
                                                         §28.13/§28.14, corrected by the second amendment)
...
**Second-amendment correction:** `recordExecutionTerminalFact`'s input is no longer a caller-supplied `terminalFact` object (which let a caller submit a persisted-shape value directly, including a self-selected `DEADLINE_VIOLATION` or a second, disagreeing `actualLatencyMs`). It now takes `{ attemptId, actualLatencyMs, result: ExecutionTerminalResultInput }` (§28.13) — `actualLatencyMs` supplied exactly once, `result` restricted to `SUCCEEDED`/`FAILED` only, with `terminalFact.kind` (including a possible `DEADLINE_VIOLATION`) always derived by the operation itself, never accepted as input.

### 28.18 Claim input and terminal-fact input — narrowest legal shape — *terminal-fact input corrected by the Slice 2D-D1-A0 second amendment*
...
| `CLAIMED` | `TERMINAL_FACT_READY` / `RESERVED` (`SUCCEEDED`/`FAILED`, as submitted, *or* `DEADLINE_VIOLATION`, derived — never as submitted, §28.13/§28.14) | `recordExecutionTerminalFact({ attemptId, actualLatencyMs, result })` |
