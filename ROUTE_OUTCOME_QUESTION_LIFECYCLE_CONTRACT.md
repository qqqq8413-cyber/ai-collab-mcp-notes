...
As written at the Slice 2D-D1-0 freeze: the domain runtime (`src/stress-test/deliberation.ts`, `applyLatencySpend` — still throws if `spent + amount > ceiling`, still charged only at terminal-outcome time) did **not** model in-flight latency reservations, atomic execution claims, or an execution checkpoint of any kind; that section documented and froze the architecture and policy for those mechanisms without implementing them.

**Status corrected by Slice 2D-D1-A.** Those mechanisms now exist in the same file as an offline execution-coordination runtime: `RouteExecutionCheckpoint`'s phase union, the atomic claim-and-reservation transaction, derived `effectiveLatencyEncumbrance`, the pre-call-failure path, checkpoint-backed `RouteOutcome` replay, and the state-level coordination boundary every latency-consuming write now enters. `applyLatencySpend` itself is unchanged, and the canonical product validators were not weakened. **This changes nothing about execution authorization: provider-backed route execution remains NOT AUTHORIZED**, unconditionally — the D1-A runtime coordinates already-obtained terminal facts and calls no provider, model, or retrieval mechanism, and that runtime is itself pending its own separate review/acceptance. What the §26.9 `LatencyBudget` race required — an actual admission implementation (§27.12) rather than documentation — is what D1-A supplies, within one process only (§28.23: never cross-process durability).
...
**Status: ACCEPTED / CLOSED, at architecture SHA `e9b55651ff1897f1ae32bb51005c874480e9dacc`.** The final bounded convergence review found no implementation-blocking contract gap; Slice 2D-D1-A0 is formally accepted/closed, and `SLICE 2D-D1-A`'s offline runtime was separately authorized on that basis (see §28.30). That runtime now exists in `src/stress-test/deliberation.ts` — the checkpoint union, `RouteExecutionCheckpointStore`, `DeliberationStateAccessPort`, `LiveExecutionPolicyResolver`, and the exact §28.17 operation set, with a deterministic in-memory reference implementation for tests. It makes zero provider/model calls. **That runtime is now itself separately ACCEPTED / CLOSED as well, at runtime acceptance SHA `1aaf7fc5874a18d0d3a79f342659436f33629b6d` (see §28.30's current-state update) — bounded exactly to deterministic single-process serialized coordination semantics, never cross-process/database/crash-durability guarantees.** **The remainder of this paragraph is the historical record of how this section reached its architecture-acceptance state.** Two schema inconsistencies found in remote review of the original freeze were corrected in place by the Slice 2D-D1-A0 amendment (never implemented, so corrected directly rather than via this document's append-only historical-correction convention, which applies only to already-accepted slices): (1) the reservation-vs-`RouteOutcome` handoff double-counted latency capacity between outcome persistence and checkpoint acknowledgement — corrected via the derived `effectiveLatencyEncumbrance` concept, §28.11/§28.12; (2) the admission-rejection shape required a `latencyLimitMs` field even when the defect *was* a missing/invalid limit — corrected by splitting `TERMINAL_FACT_READY` into `ReservedTerminalFactReadyCheckpoint`/`AdmissionRejectedTerminalFactReadyCheckpoint`, §28.13. A second amendment then corrected one remaining exact-API inconsistency: `recordExecutionTerminalFact` accepted a caller-supplied `terminalFact` object that duplicated `actualLatencyMs` (once at the top level, once nested inside it, permitting disagreement) and let a caller submit `{ kind: 'DEADLINE_VIOLATION' }` directly, even though a deadline violation is strictly a coordination-derived fact, never a caller-chosen one — corrected via the caller-facing `ExecutionTerminalResultInput` type (`SUCCEEDED`/`FAILED` only, no `actualLatencyMs`, no `DEADLINE_VIOLATION` variant), with `DEADLINE_VIOLATION` always derived by comparing the single supplied `actualLatencyMs` against the authoritative `ClaimedCheckpoint.latencyLimitMs`, §28.13/§28.14/§28.17/§28.18. A third amendment then closed five independently demonstrated architecture gaps found by adversarial review: (A) the checkpoint could not represent an accepted `SEEK_EVIDENCE` `INCONCLUSIVE` outcome — closed by adding `INCONCLUSIVE` as a third normal result kind, scoped exclusively to `SEEK_EVIDENCE`, §28.13/EC-19; (B) an active reservation and a separately-legal latency-consuming write (`recordRouteOutcome`'s other branches, and the fully independent `closeContextRequestWithoutResponse` operation — confirmed by inspecting every `applyLatencySpend` call site) could jointly overrun the ceiling because they shared no write authority — closed by a state-level coordination critical section, §28.34/EC-22; (C) a deterministic pre-call `REFERENCE_RESOLUTION` failure (already required by D1-0's failure matrix) had no legal checkpoint/API path — closed by broadening the admission-rejection shape into a canonical `PreCallFailedTerminalFactReadyCheckpoint` and a dedicated `recordExecutionPreCallFailure` operation, §28.13/§28.33/EC-21; (D) `claimRouteExecution` required live-execution authority it had no way to obtain — closed by an explicit `LiveExecutionPolicyResolver` dependency, removing caller-supplied `latencyLimitMs` entirely, §28.32/EC-20; (E) `OUTCOME_COMMITTED` permitted discarding the terminal fact that read integrity is required to re-verify against — closed by requiring `terminalFact` retention with no compaction authorized by this slice, §28.15/EC-24. Two directly-adjacent items were closed alongside these: `latencyLimitMs` now derives only from policy (folded into D), and checkpoint-backed `RouteOutcome` persistence now replays the exact stored terminal fact rather than trusting caller retyping, via `persistExecutionTerminalFactAsRouteOutcome`/`projectRouteOutcomeToTerminalFact`, §28.35/EC-23. A fourth amendment then closed three remaining coordination/API gaps found by remote read-only convergence review: (A) §28.34's "skip coordination when effective encumbrance reads zero" rule was itself racy — a concurrent claim can move encumbrance from zero to positive between that read and the write it was meant to gate, so every new latency-consuming write must now enter the state-level coordination boundary **unconditionally**, with the positive-encumbrance question answered only *inside* that boundary, §28.34/EC-26; (B) serializing operations through a critical section does not, by itself, guarantee a second operation observes the `DeliberationState` a first operation just committed — closed by an explicit `DeliberationStateAccessPort` dependency that resolves the current authoritative state *inside* the coordination boundary and commits any new state before releasing it, never trusting a caller-captured snapshot taken before the boundary was entered, §28.34/EC-27; (C) `recordExecutionPreCallFailure` had no dependency capable of enforcing §28.32's own `DISABLED → no checkpoint of any kind` rule, making it a policy-blind path that could manufacture a checkpoint while live execution was disabled — closed by requiring it to resolve the same `LiveExecutionPolicyResolver` as `claimRouteExecution` before recording any pre-call fact, §28.33/EC-28. Two bounded consistency cleanups were made alongside these: EC-9 now includes `INCONCLUSIVE` in the `actualLatencyMs <= reservedLatencyMs` rule (it was omitted when `INCONCLUSIVE` was added by the third amendment), and the `applyLatencySpend` inventory's generic-`FAILED` call site is now classified `MIXED` rather than machine-path-only, since it terminates `ADD_CONTEXT` exactly as readily as any machine route. A fifth amendment then closed two remaining implementation blockers found by a final read-only convergence review, plus two current-state consistency corrections: (A) `recordExecutionPreCallFailure` still validated attempt terminality (no existing checkpoint, no existing `RouteOutcome`) against a caller-supplied/caller-captured `DeliberationState`, so a stale snapshot could fabricate a pre-call checkpoint for an attempt a concurrent operation had already terminalized — closed by adding `DeliberationStateAccessPort` to this operation too and freezing its full 13-step current-state-resolving sequence, §28.33/EC-29; (B) a `PreCallFailedTerminalFactReadyCheckpoint`'s legitimately-zero `reservedLatencyMs`/`effectiveLatencyEncumbrance` meant budget coordination alone could not stop a direct, coordinated `recordRouteOutcome` call from landing caller-retyped terminal data for an attempt that already had a checkpoint — closed by a new checkpoint-backed exclusivity rule (§28.36): once any checkpoint exists for an `attemptId`, the only legal path to a new `RouteOutcome` is `persistExecutionTerminalFactAsRouteOutcome`'s exact replay, regardless of checkpoint phase or budget occupancy, EC-30. Two consistency corrections were made alongside these: §28.32's composition-dependency examples, which had drifted out of sync with the fourth amendment's own `DeliberationStateAccessPort` addition, were brought current; and §28.34.4's coordination-boundary participant list, which omitted `markExecutionOutcomeCommitted` despite §28.17/§28.19 already requiring it to resolve current state inside that boundary, now lists it explicitly. That fifth amendment did not itself authorize beginning `SLICE 2D-D1-A`; the final bounded verification it named — the exact API surface, EC-1–EC-30, the transition matrix, and the D1-0 guarantee mapping — has since been performed and passed, which is what produced the accepted/closed status stated at the top of this paragraph. Affected subsections are marked "*corrected by the Slice 2D-D1-A0 amendment*", "*second amendment*", "*third amendment*", "*fourth amendment*", or "*fifth amendment*" inline.
...
### 28.8 No-call admission failure — exact representation (§27.12's race, made mechanical) — *corrected by the Slice 2D-D1-A0 amendment; shape renamed/broadened by the third amendment*

**Amendment note:** the shape below replaces the amendment-superseded version of this subsection, which required a `latencyLimitMs` field on the rejection record even when the defect *was* a missing or malformed `latencyLimitMs` — an unrepresentable requirement that would have forced either fabricating a value or persisting a malformed raw one. Corrected by introducing a dedicated subtype with no `latencyLimitMs` field at all.

**Third-amendment note:** that dedicated subtype is now named `PreCallFailedTerminalFactReadyCheckpoint` (§28.13) — the name `AdmissionRejectedTerminalFactReadyCheckpoint` is retired as a current-schema term (retained only as a historical label for what this subsection's admission path produces) because the same shape is now also the canonical target of the independent `recordExecutionPreCallFailure` operation (§28.33, Blocker C), which represents pre-call failures `claimRouteExecution` itself never detects (most importantly a deterministic pre-call `REFERENCE_RESOLUTION` failure). There is exactly one current canonical pre-call-failure schema, produced by two different operations for different causes.

**Frozen:** when ownership can be claimed but the latency reservation cannot legally be granted, the transaction produces, atomically and directly, a `PreCallFailedTerminalFactReadyCheckpoint` (§28.13) — **never** a `latencyLimitMs`-bearing record, regardless of cause:
...
PreCallFailedTerminalFactReadyCheckpoint {
  ...
  terminalFact: {
    kind: 'FAILED';
    failure: FailureInfo;                 // category EXECUTION (claim-path causes, §28.9) or REFERENCE_RESOLUTION (§28.33)
    actualLatencyMs: 0;
  };
}
```

No `latencyLimitMs` field exists on this shape at all — not the caller's malformed input, not a fabricated replacement (§28.13 makes this the structural discriminant distinguishing this subtype from an ordinary executed one). No `ClaimedCheckpoint` reaches an external caller in this path. Provider calls = 0. `actualLatencyMs` = 0 (no fabrication — no external work was ever attempted). For every cause reachable through `claimRouteExecution` itself (§28.9 below), **category is always `EXECUTION`** — chosen because product references are valid, no transport call was attempted (ruling out `TRANSPORT`), no external response exists to validate (ruling out `VALIDATION`), and no reference-resolution failure occurred (ruling out `REFERENCE_RESOLUTION` *for this path specifically*); what failed is execution *policy* itself being unable to legally admit the operation, which is exactly `EXECUTION`'s definition in the already-frozen mapping (§27.9). `REFERENCE_RESOLUTION` reaches this same shape only through the separate `recordExecutionPreCallFailure` operation (§28.33). No new `FailureCategory` is invented; the closed vocabulary (§27.9) is unchanged.

**This is distinct from a policy-`DISABLED` rejection, which creates no checkpoint at all** — see §28.32.

### 28.9 Claim-path admission-rejection causes — all normalize to the same shape

Exactly three causes reach the `PreCallFailedTerminalFactReadyCheckpoint` shape (§28.8) through `claimRouteExecution` itself, all occurring strictly before any provider call, all normalizing to `FailureInfo.category = 'EXECUTION'`, distinguished only by `FailureInfo.message` wording — never a second category, never a provider-specific one:

1. **Missing or invalid, policy-derived `latencyLimitMs`** (missing, non-number, `NaN`, `Infinity`, `<= 0`, fractional) — a configuration/policy defect (§28.32 — this can no longer be a caller-input defect, since `latencyLimitMs` is no longer caller-supplied at all; it is a `LiveExecutionPolicyResolver` output defect). The malformed raw value is never stored anywhere on the checkpoint; no replacement value is fabricated.
2. **Structurally valid `latencyLimitMs` but insufficient remaining reservable budget** — a legitimate admission failure against `remainingLatency` (§27.12's formula). The checkpoint MAY optionally retain the originally-requested value for sanitized audit purposes as `requestedLatencyLimitMs` on the rejection record — but this freeze does not introduce that field: no concrete need for it was demonstrated by inspection this packet, and the smaller design (omit it) is preferred per this packet's own instruction until a future packet demonstrates otherwise. If ever introduced, it must be finite, positive, integer, request/audit data only, and never interpreted as a reservation or counted in any budget sum.
...
A fourth, categorically different cause — a deterministic pre-call `REFERENCE_RESOLUTION` failure — reaches the same persisted shape through the separate `recordExecutionPreCallFailure` operation, never through `claimRouteExecution`'s own admission logic; see §28.33.
...
- **`TERMINAL_FACT_READY` (`ReservedTerminalFactReadyCheckpoint`), exactly one authoritative `RouteOutcome` already persisted for this `attemptId`, and `projectRouteOutcomeToTerminalFact(outcome)` (§28.35, third amendment) exactly agrees with the stored `terminalFact`** — `attemptId`, `route`, `status`/`kind`, `latencyConsumed`/`actualLatencyMs`, and the route-specific terminal result required for commit integrity all agreeing, not latency alone: `effectiveLatencyEncumbrance = reservedLatencyMs - terminalFact.actualLatencyMs` (the **residual** — required invariant `0 <= effectiveLatencyEncumbrance <= reservedLatencyMs`, since `actualLatencyMs <= reservedLatencyMs` always holds for a committable terminal fact, EC-9). If no exact matching outcome exists — wrong `attemptId`, disagreeing `route`, disagreeing `latencyConsumed`, *or* an otherwise-plausible outcome whose route-specific result semantically disagrees with the checkpointed one (§28.35's exact-agreement requirement, third amendment) — this residual reduction **never** applies; the checkpoint's full `reservedLatencyMs` remains encumbered. No heuristic, no "latest outcome," no first-match behavior.
- **`TERMINAL_FACT_READY` (`PreCallFailedTerminalFactReadyCheckpoint`, §28.8/§28.13, renamed by the third amendment):** `effectiveLatencyEncumbrance = 0` (`reservedLatencyMs` is already `0` by construction).
...
### 28.13 `TERMINAL_FACT_READY` — exact shape split and terminal-fact representation — *corrected by the Slice 2D-D1-A0 amendment; broadened for `INCONCLUSIVE` and renamed by the third amendment*
...
**Third-amendment note (Blocker A):** the accepted product runtime already permits `SEEK_EVIDENCE` to terminate with `status: 'INCONCLUSIVE'` (`SeekEvidenceRouteOutcome`, §25, Slice 2D-C5-B). The checkpoint schema is corrected to represent this exactly, as a third normal result kind — never a "maybe" variant on `SUCCEEDED`, never generalized to any other route.

```
TerminalFactReadyCheckpoint =
  | ReservedTerminalFactReadyCheckpoint
  | PreCallFailedTerminalFactReadyCheckpoint    // renamed from AdmissionRejectedTerminalFactReadyCheckpoint, third amendment
...
PreCallFailedTerminalFactReadyCheckpoint {
  ...
  terminalFact: { kind: 'FAILED'; actualLatencyMs: 0; failure: FailureInfo };  // category EXECUTION (§28.9) or REFERENCE_RESOLUTION (§28.33)
}

ReservedTerminalFact =
  | { kind: 'SUCCEEDED';     actualLatencyMs: number; payload: RouteSpecificSuccessPayload }
  | { kind: 'INCONCLUSIVE';  actualLatencyMs: number; payload: SeekEvidenceInconclusivePayload }  // SEEK_EVIDENCE only, below
...
Every `ReservedTerminalFactReadyCheckpoint` is reachable only via `CLAIMED → recordExecutionTerminalFact` (§28.17) — it always carries a real `latencyLimitMs`/`reservedLatencyMs` pair from a successful claim. Every `PreCallFailedTerminalFactReadyCheckpoint` is reachable only directly from `claimRouteExecution`'s own atomic admission-rejection path (§28.8/§28.9) or from the dedicated `recordExecutionPreCallFailure` operation (§28.33) — `recordExecutionTerminalFact` can never produce one (§28.17). No malformed or fabricated `latencyLimitMs` value is ever stored on either shape.

**`INCONCLUSIVE` scope (Blocker A, resolves EC-19):** `{ kind: 'INCONCLUSIVE' }` is legal in `ExecutionTerminalResultInput`/`ReservedTerminalFact` **only** when the authoritative `RouteAttempt.route === 'SEEK_EVIDENCE'` — `recordExecutionTerminalFact` independently re-resolves `route` from the `ClaimedCheckpoint` (never trusts caller intent) and rejects an `INCONCLUSIVE` result for any other route outright, exactly mirroring the already-accepted product-domain rule (`recordRouteOutcome`'s own `if (input.status === 'INCONCLUSIVE') throw ...` guard for every non-`SEEK_EVIDENCE` route, confirmed unchanged by direct inspection this packet, `deliberation.ts:1849-1852`). `SeekEvidenceInconclusivePayload` is, per the already-established §28.13 pattern (Option A, not a new type), the exact route-specific subset of `RecordRouteOutcomeInput` already accepted for a `SEEK_EVIDENCE` `INCONCLUSIVE` outcome — confirmed by direct inspection this packet (`deliberation.ts:1909-1959`, `recordRouteOutcome`'s own `SEEK_EVIDENCE` branch): `{ result: 'INCONCLUSIVE'; evidenceSubjectId: string; citations: EvidenceCitation[] }`, excluding `attemptId`/`status`/`latencyConsumed`. No generic "maybe inconclusive" mechanism is introduced for `ADD_REVIEWER`, `REPLICATE`, or `TARGETED_PEER_CHALLENGE` — their own `AttemptStatus` allowlists are unchanged and remain `SUCCEEDED`/`FAILED` only, exactly as the product domain already enforces.

**`RouteOutcome.status` mapping, exact and closed:** `ReservedTerminalFact.kind` maps one-to-one onto `RouteOutcome.status` at checkpoint-backed commit time (§28.35) — `SUCCEEDED → SUCCEEDED`, `INCONCLUSIVE → INCONCLUSIVE`, `FAILED → FAILED`; `DEADLINE_VIOLATION` permits no `RouteOutcome` at all (§28.14). No inference from payload text or any other heuristic.

**Amendment (second) note — input shape vs. persisted shape must not be conflated:** the once-current version of this subsection let a caller submit `recordExecutionTerminalFact`'s `terminalFact` argument as if it *were* the persisted `ReservedTerminalFact` directly — meaning a caller could supply `actualLatencyMs` twice (once as the transition's own top-level input, once again inside the submitted `terminalFact`, permitting self-contradictory values), and could submit `{ kind: 'DEADLINE_VIOLATION' }` directly as if it were an ordinary caller-chosen result, even though a deadline violation is strictly a *derived* coordination fact (§28.14), never a semantic route result. Corrected by freezing a caller-supplied **input** shape, structurally distinct from the persisted `ReservedTerminalFact`:

```
ExecutionTerminalResultInput =
  | { kind: 'SUCCEEDED';    payload: RouteSpecificSuccessPayload }
  | { kind: 'INCONCLUSIVE'; payload: SeekEvidenceInconclusivePayload }  // legal only for attempt.route === 'SEEK_EVIDENCE', below
  | { kind: 'FAILED';       failure: FailureInfo }
```

`ExecutionTerminalResultInput` carries **no** `actualLatencyMs`, `latencyLimitMs`, `reservedLatencyMs`, `route`, `phase`, or parent binding — and it has **no** `DEADLINE_VIOLATION` variant at all; a caller cannot even structurally express one. `recordExecutionTerminalFact`'s full input is `{ attemptId, actualLatencyMs, result: ExecutionTerminalResultInput }` (§28.17/§28.18) — `actualLatencyMs` is supplied **exactly once**, at the top level, and the coordination runtime — never the caller — derives which persisted `ReservedTerminalFact.kind` results, by comparing that single `actualLatencyMs` against the authoritative `ClaimedCheckpoint.latencyLimitMs` it independently resolves (§28.14's derivation rule): if `actualLatencyMs > latencyLimitMs`, the persisted fact is `{ kind: 'DEADLINE_VIOLATION', actualLatencyMs }` regardless of what `result.kind` the caller submitted — including an `INCONCLUSIVE` submission, third amendment; otherwise the persisted fact is built from `result` with `actualLatencyMs` copied in exactly once, and — for `result.kind === 'INCONCLUSIVE'` — only after independently re-verifying `attempt.route === 'SEEK_EVIDENCE'` (above). No first/latest/caller-intent resolution between two disagreeing latency values is possible, because there is only ever one value to disagree with.
...
**`DEADLINE_VIOLATION` is derived, never caller-selected (corrected by the Slice 2D-D1-A0 second amendment):** a caller cannot submit `{ kind: 'DEADLINE_VIOLATION' }` as an `ExecutionTerminalResultInput` (§28.13) — no such variant exists in that input union. `recordExecutionTerminalFact` derives it, and derivation strictly **precedes** any normal result classification, in this frozen order: (1) resolve the exact, authoritative `ClaimedCheckpoint` for `attemptId`; (2) structurally validate `actualLatencyMs` (§28.16-adjacent — finite, integer, `>= 0`; malformed input is rejected outright, never stored, never substituted); (3) compare that single `actualLatencyMs` against the checkpoint's own `latencyLimitMs`; (4) if `actualLatencyMs > latencyLimitMs`, the persisted fact is `{ kind: 'DEADLINE_VIOLATION', actualLatencyMs }` **regardless of the caller's submitted `result.kind`** — a `SUCCEEDED` result whose actual latency overruns the deadline is stored as `DEADLINE_VIOLATION`, not `SUCCEEDED`; a `FAILED` result whose actual latency overruns the deadline is likewise stored as `DEADLINE_VIOLATION`, not `FAILED`; an `INCONCLUSIVE` result (third amendment, Blocker A) whose actual latency overruns the deadline is likewise stored as `DEADLINE_VIOLATION`, not `INCONCLUSIVE` — the deadline breach is coordination-layer truth that precedes and supersedes ordinary product-terminal classification once the authorized latency envelope has itself been breached, and no submitted-result kind is permitted to conceal it; (5) only when `actualLatencyMs <= latencyLimitMs` is the caller's `result` used to build a normal `SUCCEEDED`/`INCONCLUSIVE`/`FAILED` `ReservedTerminalFact` (with the `INCONCLUSIVE` route-scope check of §28.13 also applied at this step). This forecloses both failure modes this amendment was raised to prevent: a caller cannot fabricate a violation when none occurred (no input variant exists for it), and cannot conceal a real violation behind an ordinary result (the derivation runs and overrides before any caller-supplied classification is trusted).
...
### 28.15 `OUTCOME_COMMITTED` — minimum fields, `terminalFact` retention now required — *corrected by the Slice 2D-D1-A0 third amendment (Blocker E)*

**Amendment note:** the prior version of this subsection permitted `terminalFact` to be omitted from a "canonical compacted representation once committed," relying on timestamps alone to prove phase progression. This directly contradicts §28.19/§28.20/EC-12's own requirement that read integrity be able to re-verify exact `RouteOutcome` ↔ terminal-fact agreement — a corrupted/tampered `RouteOutcome` (e.g. a `REPLICATE` result silently altered from `REPRODUCED` to `NOT_REPRODUCED` after commit, with every envelope field otherwise unchanged) would be **undetectable** by timestamps alone, since timestamps carry no semantic content. Corrected:
...
OutcomeCommittedCheckpoint {
  ...
  terminalFact: ReservedTerminalFact;   // RETAINED, not compacted away — §28.13's SUCCEEDED/INCONCLUSIVE/FAILED shape
}
```

**`terminalFact` MUST remain retained on every `OutcomeCommittedCheckpoint` produced under this contract — compaction of `terminalFact` is NOT authorized by Slice 2D-D1-A0.** `actualLatencyMs` is no longer duplicated as a separate top-level field (the prior version's "convenience" copy, now redundant): a reader derives it from `terminalFact.actualLatencyMs` directly — one authoritative source, not two fields that could silently drift apart. A future, separately-authorized slice MAY design a compaction mechanism (a canonical terminal-fact digest, a retained audit snapshot, or another independently verifiable proof) once such a mechanism exists and is itself verified — but no hashing/canonicalization/digest scheme is invented here, and until that future slice exists, **payload omission after `OUTCOME_COMMITTED` is NOT AUTHORIZED**. Read integrity (§28.20) uses this retained `terminalFact`, not timestamps, to prove exact agreement with the authoritative `RouteOutcome` (§28.19/§28.35).
...
### 28.17 Exact API surface — *return/precondition types corrected by the Slice 2D-D1-A0 amendment; broadened by the third amendment (Blockers C/D/G); state-authority dependency added by the fourth amendment (Blockers B/C); pre-call state authority and checkpoint exclusivity closed by the fifth amendment (Blockers A/B)*
...
```
claimRouteExecution(session, deliberationState, store, deliberationStateAccessPort, liveExecutionPolicyResolver, input: { attemptId })
  → ClaimedCheckpoint | PreCallFailedTerminalFactReadyCheckpoint | throws
                                                        (§28.7/§28.8/§28.9/§28.32/§28.34 — never throws for an
                                                         ordinary admission failure with a checkpoint-producing
                                                         cause; DOES throw/rejects with NO checkpoint created at
                                                         all when the resolved policy is DISABLED, §28.32 — never
                                                         returns a ReservedTerminalFactReadyCheckpoint.
                                                         latencyLimitMs is no longer caller input, third amendment
                                                         — resolved internally from liveExecutionPolicyResolver,
                                                         §28.32. spent/encumbrance for admission are read from the
                                                         current state resolved via deliberationStateAccessPort
                                                         inside the state coordination boundary, never from the
                                                         caller-supplied deliberationState, fourth amendment, §28.34)

recordExecutionPreCallFailure(session, deliberationState, store, deliberationStateAccessPort, liveExecutionPolicyResolver, input: { attemptId, failure: FailureInfo })
  → PreCallFailedTerminalFactReadyCheckpoint | throws  (§28.33, third amendment, Blocker C — for pre-call failures
                                                         claimRouteExecution's own admission logic never detects,
                                                         principally a deterministic REFERENCE_RESOLUTION failure;
                                                         failure.category restricted to REFERENCE_RESOLUTION or
                                                         EXECUTION only, never TRANSPORT/VALIDATION, §28.33.
                                                         liveExecutionPolicyResolver added by the fourth amendment,
                                                         Blocker C — DISABLED throws/rejects with NO checkpoint,
                                                         mirroring claimRouteExecution exactly, §28.33.
                                                         deliberationStateAccessPort added by the fifth amendment,
                                                         Blocker A — attempt terminality (no existing checkpoint,
                                                         no existing RouteOutcome) and parent-state binding are
                                                         re-verified against the current authoritative state
                                                         resolved inside state coordination, never the caller's own
                                                         possibly-stale deliberationState, §28.33)

recordExecutionTerminalFact(store, input: { attemptId, actualLatencyMs, result: ExecutionTerminalResultInput })
  → ReservedTerminalFactReadyCheckpoint                (§28.18 — requires current phase CLAIMED; can never
                                                         produce a PreCallFailedTerminalFactReadyCheckpoint,
                                                         since admission was already resolved inside the claim;
                                                         internally derives terminalFact.kind — SUCCEEDED,
                                                         INCONCLUSIVE, FAILED, or DEADLINE_VIOLATION — from
                                                         actualLatencyMs vs. the checkpoint's own latencyLimitMs,
                                                         never from caller input; §28.13/§28.14, extended for
                                                         INCONCLUSIVE by the third amendment)

markExecutionOutcomeCommitted(session, deliberationState, store, deliberationStateAccessPort, input: { attemptId })
  → OutcomeCommittedCheckpoint                         (§28.19, tightened by the third amendment to require
                                                         exact projectRouteOutcomeToTerminalFact agreement, §28.35;
                                                         retains terminalFact on the committed checkpoint, §28.15;
                                                         reads the authoritative RouteOutcome via the current state
                                                         resolved inside coordination, fourth amendment, §28.34)

persistExecutionTerminalFactAsRouteOutcome(session, deliberationState, store, deliberationStateAccessPort, input: { attemptId })
  → DeliberationState                                  (§28.35, third amendment, Blocker G — coordination-layer
                                                         convenience operation: acquires state coordination,
                                                         resolves current state inside it via
                                                         deliberationStateAccessPort, derives RecordRouteOutcomeInput
                                                         from the stored terminal fact, calls the canonical
                                                         recordRouteOutcome against that current state, commits the
                                                         result before releasing coordination, fourth amendment,
                                                         §28.34/§28.35; never lets a caller retype status/latency/
                                                         payload/failure, and never lets a caller's own possibly-
                                                         stale deliberationState drive admission)

assertRouteExecutionCheckpointIntegrity(session, deliberationState, store, attemptId?)
  → void | throws                                      (§28.20)
```

**Amendment correction:** the amendment-superseded surface returned/accepted the single undifferentiated `TerminalFactReadyCheckpoint` at both `claimRouteExecution` and `recordExecutionTerminalFact`. Corrected: `claimRouteExecution`'s only two possible checkpoint-producing results are `ClaimedCheckpoint` (admission succeeded) or `PreCallFailedTerminalFactReadyCheckpoint` (admission failed for one of the §28.9 causes) — it never produces a `ReservedTerminalFactReadyCheckpoint` directly, because that subtype only exists after an actual external call has been attempted. `recordExecutionTerminalFact` is, symmetrically, the only operation that can ever produce a `ReservedTerminalFactReadyCheckpoint`, and it can never produce a `PreCallFailedTerminalFactReadyCheckpoint`, because by the time it runs (requiring phase `CLAIMED`), admission has already succeeded.

**Second-amendment correction:** `recordExecutionTerminalFact`'s input is no longer a caller-supplied `terminalFact` object (which let a caller submit a persisted-shape value directly, including a self-selected `DEADLINE_VIOLATION` or a second, disagreeing `actualLatencyMs`). It now takes `{ attemptId, actualLatencyMs, result: ExecutionTerminalResultInput }` (§28.13) — `actualLatencyMs` supplied exactly once, `result` restricted to `SUCCEEDED`/`INCONCLUSIVE`/`FAILED` (the last extended by the third amendment) only, with `terminalFact.kind` (including a possible `DEADLINE_VIOLATION`) always derived by the operation itself, never accepted as input.

**Third-amendment correction:** `claimRouteExecution` no longer accepts `latencyLimitMs` as ordinary caller data (Blocker D, §28.32) — it now takes a fixed `liveExecutionPolicyResolver` dependency (composition-level, never per-call caller-supplied) and resolves `latencyLimitMs` from it after resolving the authoritative `RouteAttempt`. A `DISABLED` policy result now produces **no checkpoint at all** — a plain reject/throw — distinct in kind from the three checkpoint-producing admission-rejection causes (§28.9/§28.32). `recordExecutionPreCallFailure` and `persistExecutionTerminalFactAsRouteOutcome` are new operations (Blockers C and G respectively); neither existed in the amendment-superseded surface.

**Fourth-amendment correction:** every operation whose result depends on current `latencyBudget`/`outcomes` state (`claimRouteExecution`, `markExecutionOutcomeCommitted`, `persistExecutionTerminalFactAsRouteOutcome`) now takes a `deliberationStateAccessPort` composition dependency and resolves the current authoritative `DeliberationState` from it **inside** the state coordination boundary — the caller-supplied `deliberationState` argument each already accepted is narrowed to identification/binding-verification use only (§28.3), never the accounting snapshot itself (§28.34.4). `recordExecutionPreCallFailure` now also takes `liveExecutionPolicyResolver` and can throw/reject with no checkpoint created, exactly mirroring `claimRouteExecution`'s own `DISABLED` handling (§28.33) — closing the gap where it could previously manufacture a pre-call-failure checkpoint while live execution was disabled for the attempt's route.

**Fifth-amendment correction:** `recordExecutionPreCallFailure` now also takes `deliberationStateAccessPort` (Blocker A) — its own attempt-terminality preconditions (no existing checkpoint, no existing `RouteOutcome`) and parent-state binding are re-verified against the current authoritative state resolved inside coordination, exactly like every other operation this fourth-amendment dependency already governs; a caller can no longer manufacture a pre-call checkpoint for an attempt that a concurrent operation has already terminalized, by the simple expedient of holding a stale snapshot. Separately (Blocker B), `performCoordinatedLatencyWrite` (§28.34) is strengthened with a checkpoint-exclusivity precondition, and a new subsection (§28.36) freezes the rule in full: once any execution checkpoint exists for an `attemptId`, the *only* legal path to a new `RouteOutcome` for it is `persistExecutionTerminalFactAsRouteOutcome`'s exact replay — a direct, uncheckpointed `recordRouteOutcome` call for that same `attemptId` is rejected, regardless of the checkpoint's `reservedLatencyMs`/`effectiveLatencyEncumbrance` (both legitimately `0` for a `PreCallFailedTerminalFactReadyCheckpoint`). No new operation name is introduced for this — it is a precondition on the write paths that already exist.

### 28.18 Claim input and terminal-fact input — narrowest legal shape — *terminal-fact input corrected by the Slice 2D-D1-A0 second amendment; claim input corrected by the third amendment; state-authority and pre-call-failure policy gate added by the fourth amendment*

**`claimRouteExecution` input:** `{ attemptId }` only — **`latencyLimitMs` is no longer part of the caller's input at all** (third amendment, Blocker D; see §28.32 for the full `LiveExecutionPolicyResolver` contract), and **`deliberationState` is no longer the accounting authority** (fourth amendment, Blocker B; see §28.34.4). Everything else — `route`, `deliberationStateId`, `sessionId`, hashes, `latencyLimitMs`, `reservedLatencyMs`, `claimedAt` — is derived from the authoritative `RouteAttempt` (re-resolved per §28.3), the live-execution policy resolver (a fixed dependency of the operation, never caller-supplied per call, §28.32), the deadline-violation claim-blocking check (§28.14), the **current** authoritative state resolved via `deliberationStateAccessPort` inside the state coordination boundary (fourth amendment, §28.34.4 — never the caller-supplied `deliberationState` value), and the store's own transaction (§28.7). A caller may not restate any of it. A policy-derived malformed `latencyLimitMs` (§28.9 cause 1) is validated, never stored raw and never fabricated into a replacement value — it produces a `PreCallFailedTerminalFactReadyCheckpoint` with no `latencyLimitMs` field at all (§28.13). A `DISABLED` policy result is not a checkpoint-producing outcome at all (§28.32) — the operation rejects/fails closed with no execution-coordination record created.

**`recordExecutionPreCallFailure` input (new, third amendment; policy-gated by the fourth; current-state authority added by the fifth):** `{ attemptId, failure: FailureInfo }` only. `route`, `deliberationStateId`, `claimedAt`, `terminalFactReadyAt`, `reservedLatencyMs` (`0`), and `actualLatencyMs` (`0`) are all derived — never caller-supplied; there is no caller-suppliable `authorized = true` flag of any kind. `failure.category` must be `REFERENCE_RESOLUTION` or `EXECUTION` (§28.33's restricted allowlist for this operation; `TRANSPORT`/`VALIDATION` are rejected outright, since neither a transport attempt nor an external response can exist before any call). Before recording any pre-call fact, the operation resolves the same `liveExecutionPolicyResolver` as `claimRouteExecution` for the attempt's route (fourth amendment, Blocker C, §28.33) — a `DISABLED` result rejects/fails closed with **no checkpoint created**, exactly mirroring `claimRouteExecution`; only an `ENABLED` result permits a legitimate `REFERENCE_RESOLUTION`/`EXECUTION` pre-call fact to be recorded. **Fifth amendment (Blocker A):** every terminality precondition this operation evaluates — no existing checkpoint, no existing `RouteOutcome`, valid session/state binding, route still machine-executable — is now checked against the **current** authoritative `DeliberationState`, resolved via `deliberationStateAccessPort` **inside** the state coordination boundary; the caller-supplied `deliberationState` value is used only to identify the intended state, never to decide terminality (§28.33's corrected 13-step sequence). See §28.33 for the complete precondition chain and for the causal precedence between a genuine `REFERENCE_RESOLUTION` failure and a merely-malformed policy-derived `latencyLimitMs`.

**`recordExecutionTerminalFact` input:** `{ attemptId, actualLatencyMs, result }` only, where `result: ExecutionTerminalResultInput` is restricted to `{ kind: 'SUCCEEDED', payload }`, `{ kind: 'INCONCLUSIVE', payload }` (legal only for `attempt.route === 'SEEK_EVIDENCE'`, third amendment), or `{ kind: 'FAILED', failure }` (§28.13) — structurally incapable of expressing `DEADLINE_VIOLATION` or a second `actualLatencyMs`. `route`, `claimedAt`, `latencyLimitMs`, `reservedLatencyMs`, the parent binding, and the checkpoint's current phase are all derived from the existing `ClaimedCheckpoint` record, never caller-supplied. The operation requires the checkpoint's current phase to be `CLAIMED`; any other current phase is rejected (§28.25's transition matrix). Internally, per §28.14's frozen derivation order, it resolves the `ClaimedCheckpoint`, structurally validates `actualLatencyMs` (finite, integer, `>= 0` — malformed input is rejected outright, never stored, never mutating the checkpoint, never substituted with a fabricated value), compares it against `latencyLimitMs`, and only then builds the persisted `ReservedTerminalFact` — as `DEADLINE_VIOLATION` if the comparison overruns, otherwise from the caller's `result` with `actualLatencyMs` copied in exactly once (and, for `INCONCLUSIVE`, only after re-verifying the route restriction). It always produces a `ReservedTerminalFactReadyCheckpoint` — never the pre-call-failed subtype (§28.17). Per §28.22's alias-isolation rule, `result.payload`/`result.failure` are independently snapshotted at this transition; a caller mutating the object it passed in afterward never alters the stored checkpoint.

**`persistExecutionTerminalFactAsRouteOutcome` input (new, third amendment):** `{ attemptId }` only — see §28.35 for the complete derivation-and-replay contract; a caller supplies no `status`, `latencyConsumed`, `result`, `citations`, `findingIds`, `targetRef`, `sourceRef`, `response`, `boundedExcerpt`, or `failure`, all of which are derived exclusively from the stored terminal fact.

### 28.19 Commit-acknowledgement input and integrity — narrowest legal shape, no adjacency-only trust — *tightened by the third amendment (Blocker G); state resolution corrected by the fourth amendment (Blocker B)*

**`markExecutionOutcomeCommitted` input:** `{ attemptId }` only, per this packet's own preference — a caller never simply asserts `outcomeCommitted = true`. Per the fourth amendment, every check below reads against the **current** authoritative `DeliberationState`, resolved via `deliberationStateAccessPort` inside the state coordination boundary (§28.34.4) — never the caller-supplied `deliberationState` value, which (as with `claimRouteExecution`, §28.18) serves identification/binding-verification only. The operation must independently verify, before transitioning:
...
2. exactly one `RouteOutcome` exists in the **current** `deliberationState.outcomes` for this `attemptId` (already enforced as a `recordRouteOutcome` precondition, confirmed by direct inspection this packet — `deliberation.ts:1816`'s existing "one attempt has at most one outcome" check means this is a re-verification, not a new invariant);
3. **(tightened, third amendment)** `projectRouteOutcomeToTerminalFact(outcome)` (§28.35) exactly agrees with the checkpointed `terminalFact` — for `kind === 'SUCCEEDED'`/`'INCONCLUSIVE'`, the projected route-specific result agrees field-for-field with `terminalFact.payload`; for `kind === 'FAILED'` (whether an executed failure on a `ReservedTerminalFactReadyCheckpoint` or a pre-call failure on a `PreCallFailedTerminalFactReadyCheckpoint`), `RouteOutcome.status === 'FAILED'` and its `failure` agrees with `terminalFact.failure` — value equality of the closed domain fields, never object identity, category-only agreement, or prose similarity (§28.35);
...
Only if all five hold does the transition to `OUTCOME_COMMITTED` occur, and `terminalFact` is retained on the resulting `OutcomeCommittedCheckpoint` (§28.15) — never discarded. No adjacency-only trust (i.e., "a `RouteOutcome` exists somewhere, therefore this checkpoint may commit" is insufficient without the exact-agreement check in step 3–4). This is precisely the check that detects the corruption scenario this amendment names explicitly: a checkpoint originally recording `REPLICATE` `REPRODUCED` whose authoritative `RouteOutcome` is later self-consistently altered to `NOT_REPRODUCED` (same attempt/route/target/latency/envelope) fails step 3, because `projectRouteOutcomeToTerminalFact` reconstructs `NOT_REPRODUCED` while the retained `terminalFact.payload` still reads `REPRODUCED` — a disagreement invisible to timestamps alone, exactly why §28.15 now forbids compacting `terminalFact` away.
...
- product-outcome consistency, exactly the §28.19 checks (including the third amendment's exact `projectRouteOutcomeToTerminalFact` agreement, §28.35, against the retained `terminalFact`, §28.15), whenever `phase === 'OUTCOME_COMMITTED'` — this is the mechanism that detects a post-commit `RouteOutcome` alteration such as `REPLICATE` `REPRODUCED → NOT_REPRODUCED` (§28.19's worked example), which timestamps alone cannot.

"Global before local" applies exactly as this document's prior slices have already established elsewhere. The same global-before-local posture also governs cross-aggregate latency writes (§28.34, third amendment, Blocker B): a checkpoint's local reservation legality is insufficient if an uncoordinated write elsewhere in the same `deliberationStateId` could independently push `spent` past `ceiling`.

**Scope clarification (fifth amendment, no redesign):** `assertRouteExecutionCheckpointIntegrity` is a **pure validator of the `deliberationState` snapshot it is given** — it does not itself claim to resolve "the latest live state" and takes no `deliberationStateAccessPort` dependency, because nothing in this packet's source inspection demonstrated a need for one. A caller who needs integrity checked against the current live authoritative state must first obtain that state through the state-access/coordination boundary (§28.34.4) and pass the result in — exactly the same "identify vs. authoritative" distinction §28.33/§28.35 already draw for `deliberationState` arguments elsewhere, applied here to a pure read/audit operation rather than a coordinated write decision.
...
### 28.24 Required schema summary — *corrected by the Slice 2D-D1-A0 amendment; extended by the third amendment (Blockers A/C/E)*

| State | `latencyLimitMs` required? | `reservedLatencyMs` | Effective encumbrance *before* matching outcome | Effective encumbrance *after* matching outcome | Provider call occurred? | Committable? | Legal next transition |
|---|---|---|---|---|---|---|---|
| *(absent)* — `UNCLAIMED` | — (no record) | — | — | — | No | — | → `CLAIMED`, → `TFR`/`PRE_CALL_FAILED` (via `claimRouteExecution` or `recordExecutionPreCallFailure`), or **rejects with no checkpoint at all** if the resolved policy is `DISABLED` (§28.32) |
| `CLAIMED` | Yes (`> 0`) | `=== latencyLimitMs` | `reservedLatencyMs` (no outcome possible yet) | n/a | No (about to attempt) | No | → `TFR`/`RESERVED` (any kind) via `recordExecutionTerminalFact` |
| `TFR` / `PRE_CALL_FAILED` (§28.13, renamed) | **No field at all** | `0` | `0` | `0` (never has a matching outcome to transition on) | No | Yes — as a `FAILED` outcome, exactly like any other `FAILED` terminal fact | → `OUTCOME_COMMITTED` via `markExecutionOutcomeCommitted`/`persistExecutionTerminalFactAsRouteOutcome` |
| `TFR` / `RESERVED`, `terminalFact.kind` = `SUCCEEDED` | Yes (`> 0`) | `> 0` | `reservedLatencyMs` | `reservedLatencyMs - actualLatencyMs` (residual, §28.11) | Yes | Yes | → `OUTCOME_COMMITTED` |
| `TFR` / `RESERVED`, `terminalFact.kind` = `INCONCLUSIVE` (third amendment, `SEEK_EVIDENCE` only) | Yes (`> 0`) | `> 0` | `reservedLatencyMs` | `reservedLatencyMs - actualLatencyMs` (residual, §28.11) | Yes | Yes | → `OUTCOME_COMMITTED` |
| `TFR` / `RESERVED`, `terminalFact.kind` = `FAILED` | Yes (`> 0`) | `> 0` | `reservedLatencyMs` | `reservedLatencyMs - actualLatencyMs` (residual, §28.11) | Yes | Yes | → `OUTCOME_COMMITTED` |
| `TFR` / `RESERVED`, `terminalFact.kind` = `DEADLINE_VIOLATION` | Yes (`> 0`) | `> 0` | `reservedLatencyMs` (full, always — no matching outcome is ever legal, §28.14) | n/a — never reaches "after," permanently full | Yes | **No — permanently non-committable** | none — terminal, reconciliation/operator-attention only; also blocks future claims on the same `deliberationStateId` (§28.14) |
| `OUTCOME_COMMITTED` | n/a (not stored at this phase) | n/a | n/a | `0` | Yes (or No, for a `PRE_CALL_FAILED`-sourced commit) | Already committed | none — terminal; **retains `terminalFact`** (§28.15, third amendment — no compaction) |

**Note (second amendment):** which of the `TFR`/`RESERVED` rows a `recordExecutionTerminalFact` call lands on is never caller-selected — the caller submits only `actualLatencyMs` and an `ExecutionTerminalResultInput` (`SUCCEEDED`/`INCONCLUSIVE`/`FAILED`, the middle one added by the third amendment and restricted to `SEEK_EVIDENCE`, §28.13); the coordination layer derives `SUCCEEDED`, `INCONCLUSIVE`, `FAILED`, or `DEADLINE_VIOLATION` by comparing `actualLatencyMs` against the checkpoint's own `latencyLimitMs` (§28.14).

**Note (fifth amendment):** no checkpoint phase, identity, or terminal-status kind is added by the fifth amendment — every row above is unchanged in shape. What changes is an *authority* constraint layered on top of the existing "Legal next transition" column for every `TFR` row: once any checkpoint exists for an `attemptId`, its "→ `OUTCOME_COMMITTED`" transition is reachable **only** through `persistExecutionTerminalFactAsRouteOutcome`'s exact replay (§28.36) — a direct `recordRouteOutcome` call is never a legal alternate path to the same cell, regardless of that row's own `reservedLatencyMs`/effective-encumbrance values.

**Note (third amendment):** `TFR`/`PRE_CALL_FAILED` is now reachable two ways — `claimRouteExecution`'s own atomic admission-rejection (§28.9, three causes) and the independent `recordExecutionPreCallFailure` operation (§28.33, `REFERENCE_RESOLUTION` or `EXECUTION` causes) — both produce the identical canonical shape. A `DISABLED` live-execution policy result is explicitly **not** a row in this table: it produces no checkpoint of any kind (§28.32).

### 28.25 Required transition matrix — *updated by the Slice 2D-D1-A0 amendment; extended by the third amendment; exclusivity rejection added by the fifth amendment*

**Legal:**

| From | To | Operation |
|---|---|---|
| *(absent)* | `CLAIMED` | `claimRouteExecution`, live policy `ENABLED` and admission succeeds |
| *(absent)* | `TERMINAL_FACT_READY` / `PRE_CALL_FAILED` (`FAILED`, `EXECUTION` category) | `claimRouteExecution`, live policy `ENABLED` but admission rejected for one of the three §28.9 causes — no `CLAIMED` ever exposed |
| *(absent)* | `TERMINAL_FACT_READY` / `PRE_CALL_FAILED` (`FAILED`, `REFERENCE_RESOLUTION` or `EXECUTION` category) | `recordExecutionPreCallFailure` (third amendment, §28.33) — a deterministic pre-call failure `claimRouteExecution` itself never attempted to admit |
| `CLAIMED` | `TERMINAL_FACT_READY` / `RESERVED` (`SUCCEEDED`/`INCONCLUSIVE`/`FAILED`, as submitted, *or* `DEADLINE_VIOLATION`, derived — never as submitted, §28.13/§28.14) | `recordExecutionTerminalFact({ attemptId, actualLatencyMs, result })` |
| `TERMINAL_FACT_READY` / `PRE_CALL_FAILED` (`FAILED`) | `OUTCOME_COMMITTED` | `markExecutionOutcomeCommitted` / `persistExecutionTerminalFactAsRouteOutcome`, all §28.19 checks pass |
| `TERMINAL_FACT_READY` / `RESERVED` (`SUCCEEDED`/`INCONCLUSIVE`/`FAILED`) | `OUTCOME_COMMITTED` | `markExecutionOutcomeCommitted` / `persistExecutionTerminalFactAsRouteOutcome`, all §28.19 checks pass |

**Rejected (each throws / refuses, never silently no-ops):**

| Attempted transition | Why rejected |
|---|---|
| `claimRouteExecution` or `recordExecutionPreCallFailure` when the resolved live-execution policy is `DISABLED` (generally or for this route) | §28.32/§28.33 (the latter closed by the fourth amendment, EC-28) — a pure reject/fail-closed with **no checkpoint created at all**, never a `PreCallFailedTerminalFactReadyCheckpoint`; "not authorized" is distinct in kind from "authorized but not admissible this time" |
| Claim + admission rejection, but caller expects a `ClaimedCheckpoint` back | Not a rejection of the call itself — §28.8/§28.9's `PreCallFailedTerminalFactReadyCheckpoint` is the correct, successful return value; the caller must branch on the returned shape, not assume `ClaimedCheckpoint` |
| `recordExecutionTerminalFact` attempting to produce a `PreCallFailedTerminalFactReadyCheckpoint` | Admission was already resolved atomically inside `claimRouteExecution` (§28.17); this operation can only ever produce a `ReservedTerminalFactReadyCheckpoint`, and only from phase `CLAIMED` |
| `INCONCLUSIVE` result submitted for a non-`SEEK_EVIDENCE` attempt | §28.13's route-scope rule (third amendment, EC-19) — structurally legal input, but rejected on re-verified `route`, mirroring the already-accepted `recordRouteOutcome` guard |
| Caller submitting `result: { kind: 'DEADLINE_VIOLATION' }` directly to `recordExecutionTerminalFact` | Structurally impossible, not merely rejected — `ExecutionTerminalResultInput` (§28.13) has no such variant; `DEADLINE_VIOLATION` exists only as the coordination layer's own derived output, never as caller input |
| Caller submitting a second, disagreeing `actualLatencyMs` (e.g. nested inside a `result`/`terminalFact`-shaped object) to `recordExecutionTerminalFact` | Structurally impossible — `ExecutionTerminalResultInput` carries no `actualLatencyMs` field at all; the operation's only `actualLatencyMs` is the single top-level input (§28.13/§28.18, second amendment) |
...
| `recordExecutionPreCallFailure` when a checkpoint already exists for `attemptId` | §28.33's own precondition — the operation is only for an `attemptId` with no execution-coordination record yet |
...
| Commit acknowledgement whose `projectRouteOutcomeToTerminalFact(outcome)` disagrees with the stored `terminalFact` | §28.19 check 3, tightened by the third amendment (§28.35) — a self-consistent but semantically altered `RouteOutcome` (e.g. `REPLICATE` `REPRODUCED → NOT_REPRODUCED`) fails exact-agreement, not merely envelope/latency agreement |
...
| `terminalFact` omitted/removed from an `OutcomeCommittedCheckpoint` produced under this contract | §28.15, third amendment (Blocker E) — compaction is not authorized; read integrity (§28.20) rejects a current-contract checkpoint missing it |
| A direct, non-replay `recordRouteOutcome` write for an `attemptId` that already has any execution checkpoint | §28.36, fifth amendment (Blocker B/EC-23/EC-30) — rejected regardless of checkpoint phase, `reservedLatencyMs`, or `effectiveLatencyEncumbrance`; the only legal path once a checkpoint exists is `persistExecutionTerminalFactAsRouteOutcome` |
| `recordExecutionPreCallFailure` validating attempt terminality (no existing checkpoint/`RouteOutcome`) against a caller-supplied `DeliberationState` rather than the state resolved via `DeliberationStateAccessPort` inside coordination | §28.33, fifth amendment (Blocker A/EC-29) — a stale caller snapshot can never authorize creating a pre-call checkpoint for an already-terminalized attempt |
| A latency-consuming write (any `applyLatencySpend` call site, §28.34) skipping the state-level coordination boundary entirely, for any reason including a pre-boundary read of zero effective encumbrance | §28.34's unconditional-participation rule (fourth amendment, Blocker A/EC-26) — coordination entry is never conditional on a value read before entering it |
| A write that has entered coordination but, once inside, finds positive active effective encumbrance it would exceed | §28.34.5's conservative admission policy (third amendment, Blocker B/EC-22) — the first implementation rejects/defers rather than risk `spent + effectiveLatencyEncumbrance > ceiling` |
| A coordinated operation computing admission/reservation/commit-consistency against a caller-supplied or caller-captured `DeliberationState` rather than the state resolved via `DeliberationStateAccessPort` inside the boundary | §28.34.4's stale-snapshot correction (fourth amendment, Blocker B/EC-27) |

### 28.26 Frozen design-time invariants (Slice 2D-D1-A0)

Numbered locally to this section (`EC-1`–`EC-30`) rather than merged into this document's master invariant numbering (§16 and elsewhere), because — consistent with Slice 2D-D1-0's own posture (§27.18–§27.19) — these were authored as binding requirements a future runtime slice must satisfy, not as yet-accepted guarantees about existing behavior. **Current-state note (Slice 2D-D1-A):** the offline execution-coordination runtime implementing them now exists and is covered by deterministic conformance tests, but it is pending its own separate review/acceptance, so the local numbering is deliberately retained rather than merged into the master invariant list — merging would assert a level of acceptance this document has not yet granted that runtime.
...
9. **EC-9** — *(corrected by the fourth amendment — `INCONCLUSIVE` restored to this rule)* For a `SUCCEEDED`, `INCONCLUSIVE`, or `FAILED` terminal fact, `actualLatencyMs <= reservedLatencyMs` always (§28.13); a violation is representable only as `DEADLINE_VIOLATION` (EC-10), never silently clamped into a normal fact.
...
16. **EC-16** — *(added by the Slice 2D-D1-A0 amendment)* A `PreCallFailedTerminalFactReadyCheckpoint` never carries a `latencyLimitMs` field, regardless of pre-call-failure cause — a malformed or absent limit is never stored raw, and no replacement value is ever fabricated (§28.8/§28.13).
...
18. **EC-18** — *(added by the Slice 2D-D1-A0 second amendment)* `actualLatencyMs` has exactly one caller-supplied authority at terminal-fact recording — `recordExecutionTerminalFact`'s top-level input, never duplicated inside `result` — and `DEADLINE_VIOLATION` is always derived by the coordination layer from that single value plus the authoritative `ClaimedCheckpoint.latencyLimitMs`, never caller-selected; no input shape can express `{ kind: 'DEADLINE_VIOLATION' }` or a second `actualLatencyMs` (§28.13/§28.14/§28.18).
19. **EC-19** — *(added by the Slice 2D-D1-A0 third amendment, Blocker A)* `INCONCLUSIVE` is representable as a checkpoint terminal-fact kind only for an authoritative `RouteAttempt.route === 'SEEK_EVIDENCE'`, and maps exactly to `RouteOutcome.status === 'INCONCLUSIVE'` at checkpoint-backed commit time; no other route may produce it (§28.13).
20. **EC-20** — *(added by the third amendment, Blocker D)* Live-execution authority and `latencyLimitMs` derive exclusively from the authoritative `LiveExecutionPolicyResolver` dependency, never from ordinary caller assertion; a `DISABLED` policy result yields no checkpoint at all, distinct from a checkpoint-producing admission-rejection cause (§28.32).
21. **EC-21** — *(added by the third amendment, Blocker C)* A determinate pre-call `REFERENCE_RESOLUTION` failure has a legal, zero-provider-call, zero-reservation terminal checkpoint path (`recordExecutionPreCallFailure`), distinct from — and never invented to paper over — invalid/tampered parent-state corruption, which continues to reject/fail closed without any new checkpoint fact (§28.33).
22. **EC-22** — *(added by the third amendment, Blocker B; corrected by the fourth amendment, Blocker A)* Every new write capable of increasing `deliberationState.latencyBudget.spent` (every `applyLatencySpend` call site, §28.34's inventory — not only `recordRouteOutcome`) enters the same state-level latency coordination boundary **unconditionally**, before reading or acting on the current effective encumbrance — never gated by a pre-boundary encumbrance read, which the fourth amendment demonstrated is itself racy (§28.34.3). An active positive effective encumbrance, evaluated only *inside* that boundary, cannot be bypassed by a second, uncoordinated product-write surface (§28.34).
23. **EC-23** — *(added by the third amendment, Blocker G; hardened by the fifth amendment, Blocker B)* If a checkpoint exists for an `attemptId`, **any** new `RouteOutcome` for that `attemptId` must be derived exclusively through `persistExecutionTerminalFactAsRouteOutcome`'s replay of the stored terminal fact — `status`/`latencyConsumed`/route-specific payload or `failure` are never re-supplied or retyped by a caller (§28.35), and this holds **even when `reservedLatencyMs = 0` and `effectiveLatencyEncumbrance = 0`** (§28.36) — no direct-canonical-write bypass exists regardless of the checkpoint's budget occupancy.
24. **EC-24** — *(added by the third amendment, Blocker E)* `OUTCOME_COMMITTED` retains sufficient immutable terminal semantic evidence (`terminalFact` in full, §28.15) to re-prove exact `RouteOutcome` agreement (`projectRouteOutcomeToTerminalFact`, §28.35, used by §28.19/§28.20); `terminalFact` compaction is not authorized by Slice 2D-D1-A0.
25. **EC-25** — *(added by the third amendment)* `RouteOutcome` remains sole product semantic truth; execution coordination (claims, reservations, cross-aggregate write admission) may govern write admission/concurrency without ever making checkpoint content a prerequisite for `RouteOutcome`'s own intrinsic semantic validity (§28.27, unchanged in direction).
26. **EC-26** — *(added by the fourth amendment, Blocker A)* Whether coordination applies to a new latency-consuming write is never decided by a value read outside the state-level coordination boundary — coordination participation is unconditional (§28.34.3); only the *admission* decision once inside the boundary depends on the current effective encumbrance (§28.34.5).
27. **EC-27** — *(added by the fourth amendment, Blocker B)* Every coordinated admission, reservation, or checkpoint-backed persistence decision is evaluated against the current authoritative `DeliberationState`, resolved via `DeliberationStateAccessPort` from *inside* the state coordination boundary; a caller-supplied or caller-captured `DeliberationState` value may identify the intended state and verify binding, but never serves as the accounting snapshot itself (§28.34.4).
28. **EC-28** — *(added by the fourth amendment, Blocker C)* `recordExecutionPreCallFailure` resolves the same `LiveExecutionPolicyResolver` decision `claimRouteExecution` resolves, for the same route, before recording any pre-call fact; a `DISABLED` result yields no checkpoint through this operation either, exactly as it does through `claimRouteExecution` (§28.32/§28.33).
29. **EC-29** — *(added by the fifth amendment, Blocker A)* `recordExecutionPreCallFailure` evaluates attempt terminality (no existing checkpoint, no existing `RouteOutcome`) and parent-state preconditions against the current authoritative `DeliberationState` resolved via `DeliberationStateAccessPort` from inside state coordination; stale caller state can never create a pre-call checkpoint for an already-terminalized attempt (§28.33).
30. **EC-30** — *(added by the fifth amendment, Blocker B)* Once an execution checkpoint exists for an `attemptId`, every subsequent new `RouteOutcome` for that attempt is checkpoint-backed and may be persisted only through `persistExecutionTerminalFactAsRouteOutcome`'s exact stored-terminal-fact replay; zero reservation and zero effective encumbrance never restore an uncheckpointed write path (§28.36).
...
### 28.28 Live-execution authorization as claim input — *closed by the Slice 2D-D1-A0 third amendment (Blocker D)*

`claimRouteExecution` (§28.17/§28.18) no longer reduces live-execution eligibility to "a `latencyLimitMs` was supplied" — that phrasing described an unresolved gap in the amendment-superseded draft, where the claim operation was *required* to resolve live-execution authority but had no exposed dependency capable of supplying it. Closed: per D1-0 §27.14 (unchanged) and the third amendment's `LiveExecutionPolicyResolver` (§28.32, full contract), the claim operation now resolves — from that fixed policy dependency, never from the caller's own assertion — proof that live execution is authorized both generally and for the attempt's specific route, and the exact per-call `latencyLimitMs`, before any admission/reservation logic runs. This section does not design the final product UI/config representation of that policy; it fixes the authority relationship and the exact dependency shape (§28.32).
...
### 28.30 Recommended next slice — *title broadened by the third amendment; authorization gated by the fourth amendment's convergence-review requirement; gate restated by the fifth amendment*

**Direction (unchanged): `SLICE 2D-D1-A` — EXECUTION COORDINATION / CLAIM / PRE-CALL FAILURE / LATENCY RESERVATION / COORDINATED LATENCY-WRITE GUARD OFFLINE RUNTIME**, exactly as Slice 2D-D1-0 already recommended (§27.22). The exact checkpoint entity, phase union (now including `INCONCLUSIVE` and the canonical pre-call-failure shape), API surface (now including the policy resolver, `DeliberationStateAccessPort`, `recordExecutionPreCallFailure`, `persistExecutionTerminalFactAsRouteOutcome`, and the checkpoint-backed exclusivity gate, §28.36), reservation-aggregation rule, unconditional cross-aggregate write-coordination rule, and transition matrix are now specified precisely enough to implement and test entirely offline (no provider calls), mirroring how `RouteAttempt`/`RouteOutcome` were themselves built and fully tested offline before any route's successful-result semantics existed. That runtime slice must implement, at minimum, the deterministic conformance tests anticipated in §28.31, including: (a) a two-concurrent-claims-same-`attemptId` test proving exactly one obtains a provider-call-authorized `ClaimedCheckpoint`, explicitly noting that a single-process in-memory test demonstrates API/store-contract conformance only, never cross-process production durability (§28.23); (b) a reservation-aggregation test proving two attempts against one `deliberationStateId` cannot jointly reserve more than the remaining ceiling, including a self-consistent-tamper variant where two individually-valid checkpoints' combined *effective* encumbrance exceeds the ceiling and global read integrity (§28.20) rejects it; (c) an admission-failure test proving the exact §28.8 transition is followed, with zero provider calls and no permanently-ambiguous `CLAIMED` state produced for a failure that was already known and pre-call; and the additional third-, fourth-, and fifth-amendment-specific tests listed in §28.31.

**Authorization status: GRANTED. Slice 2D-D1-A0 is ACCEPTED / CLOSED at architecture SHA `e9b55651ff1897f1ae32bb51005c874480e9dacc`, and `SLICE 2D-D1-A`'s offline runtime is separately authorized.** Five successive amendment rounds corrected independently-demonstrated gaps in the same not-yet-implemented schema; the final bounded convergence review then re-checked the exact API surface, `EC-1`–`EC-30`, the transition matrix, and the D1-0 guarantee mapping (atomic claim; no automatic external retry; determinate failure terminalization; checkpoint-before-`RouteOutcome`; persistence-only reconciliation; latency reservation; explicit live authorization; pre-call `REFERENCE_RESOLUTION`; no fabricated latency; no provider replay; `RouteOutcome` remains product semantic truth — all eleven representable) and found no implementation-blocking contract gap. That review is what accepted/closed this slice and authorized the offline runtime, which now exists in `src/stress-test/deliberation.ts` with the conformance tests §28.30(a)–(c) and §28.31 require. **Two limits on what that grant means.** First, the authorization covers the OFFLINE runtime only: provider-backed route execution, retrieval, `ADD_REVIEWER` acquisition, `REPLICATE` external rerun, `TARGETED_PEER_CHALLENGE` execution, Round 2, and M2-B all remain **NOT AUTHORIZED**, unchanged (§25, §27.19). **Current-state update (Product Integration P1 packet): `SLICE 2D-D1-A`'s offline runtime is now itself ACCEPTED / CLOSED, at runtime acceptance SHA `1aaf7fc5874a18d0d3a79f342659436f33629b6d`** (`src/stress-test/deliberation.ts`, hardened by the D1-A Amendment 1 checkpoint-snapshot-isolation/coordinated-write-identity correction). This acceptance is bounded exactly as this section has stated throughout: **D1-A proves deterministic single-process serialized coordination semantics only.** It does **not** prove true simultaneous concurrency, cross-process exclusion, database transaction isolation, crash durability, or distributed locking — those remain properties a future, separately-authorized production persistence adapter must supply and separately demonstrate (§28.23, unchanged); none is built or claimed here. A zero-finding review is evidence of convergence, not proof of correctness; it does not close the possibility of a future amendment.
...
**Admission-rejection-cause tests (proves §28.8/§28.9/§28.13), independently, for each of the three claim-path causes:** missing `latencyLimitMs`; structurally invalid `latencyLimitMs` (non-number, `NaN`, `Infinity`, `<= 0`, fractional); and a structurally valid `latencyLimitMs` exceeding remaining reservable budget. Each must prove: provider calls = 0; the resulting checkpoint is a `PreCallFailedTerminalFactReadyCheckpoint` (named `AdmissionRejectedTerminalFactReadyCheckpoint` in this amendment's own original text, renamed by the third amendment, §28.13) carrying **no** `latencyLimitMs` field at all (present with any value, malformed or otherwise, fails the test); `reservedLatencyMs === 0`; `terminalFact.actualLatencyMs === 0`; `terminalFact.failure.category === 'EXECUTION'`.

**Deadline-violation claim-blocking test (proves §28.14/EC-17):** given a `deliberationStateId` with one unresolved `DEADLINE_VIOLATION` checkpoint, a subsequent `claimRouteExecution` call for a different `attemptId` on the same state must produce a `PreCallFailedTerminalFactReadyCheckpoint` (cause 3, §28.9) and never a `ClaimedCheckpoint`, even when that new attempt's own policy-resolved `latencyLimitMs` and remaining budget would otherwise admit it.

**No-duplicate-latency-source test (proves §28.13/§28.18/EC-18, Slice 2D-D1-A0 second amendment):** the `recordExecutionTerminalFact` input type must expose exactly one field capable of carrying `actualLatencyMs` — its own top-level input — with no legal path (nested inside `result`, or any other field) through which a caller can supply a second, potentially disagreeing latency value. A plain-JS excess-key rejection at the `result` boundary (§28.13's `ExecutionTerminalResultInput` accepting only `kind`+`payload` or `kind`+`failure`) is sufficient proof; the persisted `ReservedTerminalFact.actualLatencyMs` must always trace to that single input value, never a second one.

**Deadline-derived-over-success test (proves §28.14, second amendment):** given `ClaimedCheckpoint.latencyLimitMs = 100` and an input of `actualLatencyMs = 101` with `result = { kind: 'SUCCEEDED', payload: <otherwise-valid> }`, the required stored result is `terminalFact = { kind: 'DEADLINE_VIOLATION', actualLatencyMs: 101 }` — never `SUCCEEDED` — and no `RouteOutcome` commit is ever permitted for it (§28.19 step 1).

**False-violation-impossible test (proves §28.13, second amendment):** there is no legal `ExecutionTerminalResultInput` variant with `kind: 'DEADLINE_VIOLATION'` — a caller cannot request a violation when `actualLatencyMs <= latencyLimitMs`. Prove this both structurally (the input union has no such member — a plain-JS caller attempting `{ kind: 'DEADLINE_VIOLATION' }` is rejected by the same excess/invalid-`kind` check that rejects any other unrecognized `kind`) and behaviorally (an `actualLatencyMs` at or below `latencyLimitMs` with any accepted `result` never produces a stored `DEADLINE_VIOLATION`).

**Failure-overrun test (proves §28.14, second amendment):** given `latencyLimitMs = 100` and an input of `actualLatencyMs = 110` with `result = { kind: 'FAILED', failure: <otherwise-valid> }`, the required stored terminal fact is `DEADLINE_VIOLATION`, not `FAILED` — the deadline breach is coordination truth that precedes and overrides normal product-terminal classification once the authorized latency envelope has itself been breached; no normal `FAILED` `RouteOutcome` may be committed for this attempt.

**Third-amendment required tests, extending the above:**

**`INCONCLUSIVE` lifecycle test (proves §28.13/EC-19, Blocker A):** a `SEEK_EVIDENCE` attempt → `CLAIMED` → `recordExecutionTerminalFact` with `result = { kind: 'INCONCLUSIVE', payload }` → `terminalFact.kind === 'INCONCLUSIVE'` → `persistExecutionTerminalFactAsRouteOutcome` → `RouteOutcome.status === 'INCONCLUSIVE'` → `markExecutionOutcomeCommitted` → later `assertRouteExecutionCheckpointIntegrity` passes. A non-`SEEK_EVIDENCE` attempt submitting `result = { kind: 'INCONCLUSIVE', ... }` to `recordExecutionTerminalFact` must be rejected.

**`INCONCLUSIVE`-tamper test (proves §28.19/§28.35 exact-agreement, Blocker A+G):** starting from the committed lifecycle above, alter the authoritative `RouteOutcome` self-consistently from `INCONCLUSIVE` to `SUCCEEDED`/`SUPPORTIVE` (same attempt/route/envelope/latency) — later `assertRouteExecutionCheckpointIntegrity` must reject, because `projectRouteOutcomeToTerminalFact` reconstructs `SUCCEEDED`/`SUPPORTIVE` while the retained `terminalFact.kind` still reads `INCONCLUSIVE`.

**Pre-call `REFERENCE_RESOLUTION` test (proves §28.33, Blocker C):** a valid machine-route `RouteAttempt` whose required authoritative execution reference no longer resolves → `recordExecutionPreCallFailure({ attemptId, failure: { category: 'REFERENCE_RESOLUTION', message } })` → provider calls = 0 → no `ClaimedCheckpoint` ever exposed → `reservedLatencyMs === 0` → `terminalFact.actualLatencyMs === 0` → `persistExecutionTerminalFactAsRouteOutcome` may record a generic `FAILED` `RouteOutcome` from the exact checkpointed fact → `OUTCOME_COMMITTED` after exact agreement. No provider execution at any point.

**Live-policy tests (prove §28.32, Blocker D), independently:**
- **A.** Policy `DISABLED` (generally) → `claimRouteExecution` throws/rejects → no checkpoint of any kind exists for `attemptId` afterward → no reservation → no provider call.
- **B.** Policy `ENABLED` for a different route only → same as A for this attempt's route.
- **C.** Policy `ENABLED` for this route with a valid `latencyLimitMs` → the resulting `ClaimedCheckpoint.latencyLimitMs` equals exactly the policy-resolved value → the caller's input has no field capable of overriding it (`{ attemptId }` only, §28.18).
- **D.** Policy `ENABLED` but its own `latencyLimitMs` is missing/malformed → `PreCallFailedTerminalFactReadyCheckpoint`, `EXECUTION` category, provider calls = 0, no malformed value stored.
- **E.** A caller attempting to pass an extra `latencyLimitMs` field to `claimRouteExecution`'s `{ attemptId }` input → rejected as an unexpected field (`assertExactKeys`-style rejection, mirroring this document's established pattern).

**Cross-aggregate latency-coordination tests (prove §28.34, Blocker B), independently:**
1. `ceiling = 100`; attempt A reserves the full 100 (positive active effective encumbrance); an unrelated, uncheckpointed latency-consuming write of `1` (e.g. via `closeContextRequestWithoutResponse`, or any other `applyLatencySpend` call site not participating in A's claim) for the same `deliberationStateId` must be rejected/deferred **before** it mutates `deliberationState`.
2. Once A releases/commits such that capacity exists (effective encumbrance drops via §28.11/§28.12's residual/zero rules), the previously-deferred write may then legally proceed.
3. Two simultaneous operations — a new claim and an uncoordinated latency-consuming write — against the same `deliberationStateId`: the serialized outcome (whichever wins the state coordination critical section first) must never let the other proceed against a stale `remainingLatency`/`spent` read; both legal interleavings (§28.34) must be tested.
4. A global self-consistent-tamper variant: hand-construct state where `spent + SUM(effectiveLatencyEncumbrance) > ceiling` — `assertRouteExecutionCheckpointIntegrity`'s global reservation check (§28.20) must reject it.
5. Every `applyLatencySpend` call site inventoried in §28.34.2 (all seven branches of `recordRouteOutcome`, plus `closeContextRequestWithoutResponse`) has a corresponding coordination test, or a documented proof that it structurally cannot execute while positive active machine coordination exists for the same state (e.g. a `STOP`ped state, or a route that can never have a checkpoint).

**Exact-replay tests (prove §28.35, Blocker G):** given a checkpoint with `terminalFact.actualLatencyMs = 50`, no legal checkpoint-backed persistence path (`persistExecutionTerminalFactAsRouteOutcome`) may produce a `RouteOutcome.latencyConsumed` of `50.5`, `51`, or `0` — the value must be replayed exactly. The same requirement applies to `FailureInfo.message`, any route-specific result enum, `citations`, `findingIds`, `targetRef`/`sourceRef`, `boundedExcerpt`, and `response` — every one must be replayed from the checkpoint snapshot for a checkpoint-backed write, never retyped by a caller.

**Compaction tests (prove §28.15/EC-24, Blocker E):** an `OutcomeCommittedCheckpoint` produced under this contract must retain `terminalFact` — a manually-compacted checkpoint missing it, if created under the new contract, must be rejected by read integrity (§28.20). Historical states predating execution checkpoints entirely remain unaffected by this rule (§28.21, unchanged).

**Fourth-amendment required tests, extending the above:**

**Zero-encumbrance claim-vs-write race test (proves §28.34.3/EC-26, Blocker A):** starting from `S0` with `spent = 0`, `ceiling = 100`, no active encumbrance, run concurrently: A = a claim requiring reservation 100; B = an uncheckpointed latency write of 1. Both must enter the state coordination boundary even though `S0` initially shows zero encumbrance — a test that lets either operation bypass coordination on the strength of a pre-boundary zero reading must fail. Exactly one legal ordering occurs (§28.34.7's Case A or Case B); `spent(1) + encumbrance(100) = 101` must never be observed.

**Stale-snapshot test (proves §28.34.4/EC-27, Blocker B):** hand two callers the identical, old `S0` reference. B enters coordination first (`S0 → S1`, `spent = 1`, committed via `DeliberationStateAccessPort`). A enters second and must resolve `S1` inside coordination — it must not perform its admission check against its own caller-captured `S0`. This test must fail against any implementation that merely mutex-serializes closures which retain a stale state argument rather than re-resolving current state inside the boundary.

**Policy-disabled pre-call test (proves §28.33/EC-28, Blocker C):** with the live-execution policy `DISABLED` for the attempt's route, a caller attempts `recordExecutionPreCallFailure` with a `REFERENCE_RESOLUTION` failure — required: the call rejects; no checkpoint exists afterward for `attemptId`; no `RouteOutcome` is created; no reservation is created; provider calls = 0. The dedicated pre-call API must not be usable to bypass live-execution authorization that `claimRouteExecution` itself already respects.

**Policy-enabled pre-call test (proves §28.33, third-amendment lifecycle preserved):** with the live-execution policy `ENABLED` for the attempt's route, a genuine deterministic `REFERENCE_RESOLUTION` pre-call failure still produces a `PreCallFailedTerminalFactReadyCheckpoint` with `reservedLatencyMs = 0`, `actualLatencyMs = 0`, and provider calls = 0 — the third amendment's own lifecycle (§28.31's earlier "pre-call `REFERENCE_RESOLUTION` test") is unaffected by adding the policy gate.

**Fifth-amendment required tests, extending the above:**

**Stale pre-call snapshot test (proves §28.33's 13-step sequence/EC-29, Blocker A):** caller `S0` shows attempt A with no `RouteOutcome`; the authoritative current state `S1` (already committed by a concurrent operation) shows A already has one. Calling `recordExecutionPreCallFailure(..., callerS0, attemptId: A, ...)` must: resolve `S1` through `DeliberationStateAccessPort` inside coordination; reject before any checkpoint is created; leave the checkpoint count for A unchanged; leave A's `RouteOutcome` unchanged; make zero provider calls. The test must fail against any implementation that validates terminality against the caller-supplied `S0`.

**Pre-call replay-bypass test (proves §28.36/EC-23/EC-30, Blocker B):** create a `PreCallFailedTerminalFactReadyCheckpoint` for attempt A with `failure = { category: 'REFERENCE_RESOLUTION', message: 'X' }`, `actualLatencyMs = 0`. Attempt a direct, coordinated `recordRouteOutcome({ attemptId: A, status: 'FAILED', latencyConsumed: 0, failure: { category: 'EXECUTION', message: 'Y' } })` — required: rejected **before** any product-domain mutation occurs. Then call `persistExecutionTerminalFactAsRouteOutcome(A)` — required: it records exactly `REFERENCE_RESOLUTION`/`X` with `latencyConsumed = 0`, with no caller retyping surviving anywhere in the resulting `RouteOutcome`.

**Claim-admission-failure replay-bypass test (proves §28.36/EC-23, Blocker B — the zero-reservation case reached via the other pre-call-failure source):** create a `PreCallFailedTerminalFactReadyCheckpoint` for an attempt through `claimRouteExecution`'s own admission-failure path instead (`failure.category = 'EXECUTION'`, `reservedLatencyMs = 0`, `actualLatencyMs = 0`). Attempt a direct generic `FAILED` `RouteOutcome` write for the same attempt — required: rejected, because a checkpoint exists for it, exactly as in the `recordExecutionPreCallFailure`-sourced case. The zero-reservation shape does not weaken `EC-23`/`EC-30` regardless of which operation created the checkpoint.

**Legacy no-checkpoint compatibility test (proves §28.21/§28.36.4 unchanged):** given an accepted historical/offline `RouteAttempt` with no execution checkpoint at all, a newly-recorded `RouteOutcome` for it may still use the ordinary canonical `recordRouteOutcome` path directly, under the state-level latency coordination boundary (§28.34) — no retroactive checkpoint synthesis is performed, and no accepted historical fixture requires any change.

### 28.32 Live-execution policy resolver & `latencyLimitMs` authority (Slice 2D-D1-A0 third amendment, Blocker D)

**Frozen dependency:** `LiveExecutionPolicyResolver` — an infrastructure/composition-level dependency of the execution-coordination layer, never product truth, never provider selection, and never a fact the ordinary per-call caller is trusted to invent. It is resolved once per `claimRouteExecution` call, after the authoritative `RouteAttempt` is resolved, and its result is never accepted as caller input. **Fourth amendment:** the identical resolver and resolution order is now also required by `recordExecutionPreCallFailure` (§28.33) before it may record any pre-call fact — there is exactly one live-execution-authority dependency in this architecture, never two independently-maintained ones for the two operations that can produce a `PreCallFailedTerminalFactReadyCheckpoint`.

**Minimum conceptual policy result, for one authoritative `RouteAttempt`:**

```
LiveExecutionPolicyDecision =
  | { status: 'DISABLED' }
  | { status: 'ENABLED'; latencyLimitMs: number }   // finite integer > 0
```

The resolver must be capable of deciding, at minimum: (a) whether Stress Test live execution is enabled generally; (b) whether the attempt's specific route is enabled; (c) the exact per-call `latencyLimitMs` to authorize. No provider/model choice is made by or through this resolver (§27.44/§28.29, unchanged).

**`DISABLED` — pure rejection, no checkpoint (resolves §28.8's own cross-reference, closes EC-20):** if the resolver returns `DISABLED` — for Stress Test generally, or for the attempt's specific route — `claimRouteExecution` rejects/fails closed with **provider calls = 0, no latency reservation, and no checkpoint created at all** — not a `ClaimedCheckpoint`, and not a `PreCallFailedTerminalFactReadyCheckpoint` either. This is categorically distinct from the three checkpoint-producing admission-rejection causes (§28.9): those represent "this operation was authorized to be attempted but could not be legally admitted this time" (a fact worth checkpointing for audit); `DISABLED` represents "this operation was never authorized to be attempted at all," which this document's own established posture (§24 of this packet, §28.33 below) treats as a plain precondition rejection, never a fabricated execution fact invented to paper over an authorization gap.

**`ENABLED` but invalid policy-derived `latencyLimitMs`:** a determinate pre-call execution-configuration failure — reaches the ordinary `PreCallFailedTerminalFactReadyCheckpoint` shape (§28.8/§28.9 cause 1, `EXECUTION` category), because in this case the operation *was* authorized to be attempted, but the policy infrastructure supplying its parameters is itself broken. The malformed policy-derived value is never persisted; no replacement is fabricated.

**Caller input correction (resolves Blocker D concretely) — examples corrected by the fifth amendment (Consistency Correction C):** `claimRouteExecution`'s caller-facing input is `{ attemptId }` only (§28.18) — `latencyLimitMs` is never restated by the caller, closing both the missing-live-authorization gap and the caller-selected-latency-ceiling gap in one correction. Conceptually, the store, the state-access port, and the resolver are all fixed dependencies of the operation — acceptable as `RouteExecutionCoordinator { store, deliberationStateAccessPort, liveExecutionPolicyResolver }` with `coordinator.claimRouteExecution(session, deliberationState, { attemptId })`, or as a plain function `claimRouteExecution(session, deliberationState, store, deliberationStateAccessPort, liveExecutionPolicyResolver, { attemptId })` consistent with this document's established function-style API surface (§28.17) — **any current-state example must include every dependency the exact API surface requires; the amendment-superseded versions of both examples omitted `deliberationStateAccessPort`, added by the fourth amendment, and are corrected here.** Exact implementation class/function packaging remains an implementation choice; the authority relationship itself (each dependency is composition-level, never per-call caller data) is not optional.

**Live authorization ≠ human revision authority (restated, unchanged from D1-0 §27.43):** a `LiveExecutionPolicyResolver` decision means only "this external route mechanism may execute" — it never grants `HumanAdjudication`/`ACTION_CHANGE = YES`/`RevisionAction` authority, which remains entirely separate and unaffected.

**Execution coordinator's full composition-level dependency set, consolidated (fourth amendment):** `RouteExecutionCheckpointStore` (§28.2), `DeliberationStateAccessPort` (§28.34.4), and `LiveExecutionPolicyResolver` (this subsection) — no provider/model adapter exists among them, and none is added by this amendment (§28.29, unchanged). Every operation in §28.17's API surface draws on exactly the subset of these three it needs; no operation is given a dependency it has no use for.

### 28.33 Pre-call failure operation, category scope, and the route-readiness/corruption distinction (Slice 2D-D1-A0 third amendment, Blocker C; live-policy-gated by the fourth amendment; current-state authority closed by the fifth amendment, Blocker A)

**Frozen dedicated operation:**

```
recordExecutionPreCallFailure(session, deliberationState, store, deliberationStateAccessPort, liveExecutionPolicyResolver, input: { attemptId, failure: FailureInfo })
  → PreCallFailedTerminalFactReadyCheckpoint | throws
```

**Fifth-amendment note — stale-snapshot bypass, demonstrated precisely:** the fourth-amendment version of this operation gated live-execution authority correctly but still evaluated every *terminality* precondition (no existing checkpoint, no existing `RouteOutcome`, valid session/state binding) against the caller-supplied `deliberationState` value — never re-resolved as current. Counterexample: a caller holds `S0` in which attempt A has no `RouteOutcome`; concurrently, another operation commits `S1` in which A already has one; the caller then calls `recordExecutionPreCallFailure` for A using stale `S0` — if the operation trusts `S0`, "no `RouteOutcome` exists" reads true, and a pre-call checkpoint is fabricated for an attempt whose execution history is already closed. A later integrity rejection (§28.20) is insufficient — the write boundary itself must reject, before any checkpoint is created. Corrected below by extending the fourth amendment's own principle (originally scoped to numeric `latencyBudget`/`outcomes` accounting) to **any** current-state fact an execution-coordination operation's correctness depends on, including attempt/outcome/checkpoint existence and execution terminality, not only budget arithmetic.

**Frozen 13-step coordination sequence (fifth amendment):** (1) use the caller-supplied `deliberationState` only to identify the intended state and to verify the caller's own binding expectation — never as the terminality authority; (2) acquire state-level coordination for `deliberationStateId` (§28.34.4); (3) resolve the **current** authoritative `DeliberationState` via `deliberationStateAccessPort`, from inside that boundary; (4) re-verify the session/state binding against this current state (`verifyDeliberationBinding`); (5) resolve the exact current `RouteAttempt` for `attemptId` against the current state's `attempts`; (6) verify the current state carries no `RouteOutcome` for `attemptId`; (7) verify no checkpoint already exists for `attemptId` (an identity lookup against the store, §28.1); (8) verify the route is machine-executable (§27.3); (9) resolve `liveExecutionPolicyResolver`'s decision for that route; (10) if `DISABLED`, reject/fail closed with no checkpoint created; (11) if `ENABLED`, validate the supplied `failure` is a legitimate pre-call fact (category scope, below); (12) atomically create the `PreCallFailedTerminalFactReadyCheckpoint` (§28.10's one-checkpoint-per-`attemptId` atomicity); (13) release coordination. No caller-captured state may decide terminality at any step — steps 4 through 8 all read the state resolved in step 3, never the caller's own `deliberationState`. On success it derives `route`, `deliberationStateId`, `claimedAt`/`terminalFactReadyAt` (same instant), `reservedLatencyMs = 0`, and `actualLatencyMs = 0` — none of these are caller-supplied — and clones the supplied `failure` snapshot (§28.22's alias-isolation rule).

**Live-policy gate, closed by the fourth amendment (the bypass this amendment names precisely):** §28.32 already froze `DISABLED → no checkpoint of any kind` as a claim-path rule, but the third-amendment version of `recordExecutionPreCallFailure` had no dependency capable of enforcing that same rule — it was a policy-blind public path that could manufacture a legitimate-looking `PreCallFailedTerminalFactReadyCheckpoint` even while live execution was disabled for the attempt's route, silently bypassing the one authorization boundary `claimRouteExecution` itself already respected. **Corrected:** before recording any pre-call fact, `recordExecutionPreCallFailure` resolves the authoritative `RouteAttempt`, then resolves `liveExecutionPolicyResolver`'s decision for that attempt's route — exactly the same dependency and the same resolution `claimRouteExecution` performs (§28.32) — never a caller-supplied `authorized = true` flag or any other caller assertion. If `DISABLED`, the operation rejects/fails closed with **no checkpoint created**, mirroring `claimRouteExecution`'s own `DISABLED` handling precisely. Only if `ENABLED` may a legitimate `REFERENCE_RESOLUTION`/`EXECUTION` pre-call fact be recorded.

**Pre-call failure vs. `latencyLimitMs` — independent of reservation (fourth amendment):** a pre-call `REFERENCE_RESOLUTION` failure requires live execution to be `ENABLED` for the route, but it does **not** require a valid, reservable `latencyLimitMs` — no provider call will ever occur for this checkpoint, so no reservation is ever created either way (`reservedLatencyMs = 0`, unconditionally, as already frozen). If the policy is `ENABLED` but its own `latencyLimitMs` is separately malformed, **and** a genuine `REFERENCE_RESOLUTION` failure has already been independently established for the same attempt, D1-0's own causal-precedence rule (§27.9's "earliest authoritative causal failure," restated) applies unchanged: `REFERENCE_RESOLUTION` is recorded, not `EXECUTION` — the reference could never have resolved regardless of the policy's own latency configuration, so the reference failure is causally prior. If the *only* failure present is the malformed policy-derived latency configuration itself (no reference-resolution defect), the resulting checkpoint (via `claimRouteExecution`'s own admission path, §28.9 cause 1 — not this operation) is `EXECUTION`, exactly as already frozen.

**Category scope, restricted (resolves this packet's own §20 instruction):** `recordExecutionPreCallFailure` accepts exactly `REFERENCE_RESOLUTION` or `EXECUTION` as `failure.category` — the two categories this document can currently demonstrate a legitimate pre-call cause for. `TRANSPORT` is rejected outright: no transport channel was ever engaged before any call, so a transport-shaped failure cannot yet exist. `VALIDATION` is likewise rejected: it exists only to describe a received-but-unnormalizable external response (§27.9), and no external response can exist before a call was ever attempted. If a future packet's inspection demonstrates a legitimate pre-call `VALIDATION` case, it must be documented and the allowlist explicitly widened then — this freeze does not widen it speculatively.

**Route-readiness vs. product-corruption distinction (this packet's own §24, frozen explicitly):** two categorically different situations must never be conflated into the same checkpoint-producing path:

- **(A) A valid attempt whose required authoritative execution reference cannot be resolved** — e.g. a `TARGETED_PEER_CHALLENGE` attempt whose `targetRef`/`sourceRef` no longer resolves, or a `REPLICATE`/`SEEK_EVIDENCE` attempt whose bound reference has since been superseded. This is exactly `recordExecutionPreCallFailure`'s intended case: a legitimate, auditable `REFERENCE_RESOLUTION` terminal pre-call failure.
- **(B) Invalid/tampered parent state** — corrupt session binding, a duplicate checkpoint already present, an attempt already terminalized, a `STOPPED` `DeliberationState`, or any other structurally corrupt ledger condition. These are **not** execution facts about the attempt; they are preconditions of `recordExecutionPreCallFailure` itself (see above), and a violation **rejects/fails closed with no new checkpoint fact invented** — exactly the same posture this document already applies everywhere else to tampered/corrupt state (e.g. `verifyDeliberationBinding`'s own unconditional throw). Never terminalize (B) as if it were (A); doing so would fabricate an audit trail for a condition that was never a legitimate execution attempt at all.

### 28.34 Cross-aggregate latency write coordination (Slice 2D-D1-A0 third amendment, Blocker B; hardened by the fourth amendment, Blockers A and B; participant list and exclusivity precondition corrected by the fifth amendment)

**28.34.1 The regression, demonstrated:** the execution-coordination store cannot safely reserve latency capacity if a second, legal product-domain operation can independently increase `deliberationState.latencyBudget.spent` without participating in the same coordination boundary. Worked example, `ceiling = 100`, `spent = 0`: attempt A obtains a `ClaimedCheckpoint` with `reservedLatencyMs = 100` (full effective encumbrance, §28.11). A second, entirely unrelated, **uncheckpointed** product-domain write — e.g. `closeContextRequestWithoutResponse` closing an `ADD_CONTEXT` attempt with `latencyConsumed = 1` — is, in isolation, a perfectly legal call against `applyLatencySpend`: `0 + 1 <= 100` passes its own local check. But the moment it succeeds, global truth becomes `spent(1) + effectiveEncumbrance_A(100) = 101 > ceiling` — an actual, real overrun, not merely an accounting artifact like §28.11's handoff case, because neither write is aware of the other's claim on the same capacity.

**28.34.2 Full `applyLatencySpend` call-site inventory (source-grounded, this packet's own required inspection) — classification corrected by the fourth amendment:** a search of every call site in `src/stress-test/deliberation.ts` (confirmed via `grep -na` — this file contains intentional literal `\x00` delimiter bytes elsewhere that make a plain-text `grep` silently treat it as binary and report zero matches; `-a` forces text mode) found **exactly eight**, all within two functions:

| # | Line | Function | Branch | Classification |
|---|---|---|---|---|
| 1 | 1889 | `recordRouteOutcome` | generic `FAILED`, any route | **MIXED** *(corrected, fourth amendment)* — `input.status === 'FAILED'` is checked before any route branch (confirmed by direct re-inspection this packet, `deliberation.ts:1841`), so this single call site terminates `ADD_REVIEWER`/`REPLICATE`/`SEEK_EVIDENCE`/`TARGETED_PEER_CHALLENGE` exactly as readily as `ADD_CONTEXT` — never exclusively a checkpoint-backed machine-path site |
| 2 | 1939 | `recordRouteOutcome` | `SEEK_EVIDENCE` success (`SUCCEEDED` or `INCONCLUSIVE`) | Machine (checkpoint-backed) |
| 3 | 2022 | `recordRouteOutcome` | `ADD_REVIEWER` success | Machine (checkpoint-backed) |
| 4 | 2083 | `recordRouteOutcome` | `ADD_CONTEXT` `SUPPLIED` success | Non-machine (dedicated closure) |
| 5 | 2106 | `recordRouteOutcome` | `ADD_CONTEXT` `DECLINED` success | Non-machine (dedicated closure) |
| 6 | 2152 | `recordRouteOutcome` | `TARGETED_PEER_CHALLENGE` success | Machine (checkpoint-backed) |
| 7 | 2192 | `recordRouteOutcome` | `REPLICATE` success | Machine (checkpoint-backed) |
| 8 | 2934 | `closeContextRequestWithoutResponse` | `ADD_CONTEXT` `NO_RESPONSE` dedicated closure (Slice 2D-C4) | Non-machine (dedicated closure) |

**Why the per-call-site label is not load-bearing (fourth amendment):** call site 1's `MIXED` status confirms the coordination rule below cannot be keyed on "which source line called `applyLatencySpend`," or on any static route/branch classification at all — the real question, asked fresh at every write, is purely behavioral: **"can this new operation increase `deliberationState.latencyBudget.spent`?"** Every one of the eight call sites answers yes, so every one participates in the coordination boundary below unconditionally (§28.34.3) — the table above is a completeness inventory (no hidden ninth call site exists), not a routing table for deciding who coordinates and who doesn't.

**28.34.3 Frozen rule — unconditional coordination participation, corrected by the fourth amendment (Blocker A):** the amendment-superseded version of this rule coordinated a new latency-consuming write only when the `deliberationStateId` currently carried positive active effective encumbrance, and let a write proceed uncoordinated when a pre-check read zero. **This is itself racy** — the exact counterexample this amendment names: `ceiling = 100`, `spent = 0`, encumbrance = 0; write B reads encumbrance as zero and chooses the uncoordinated path; concurrently, claim A enters coordination, independently observes `spent = 0`, and reserves 100; B then completes its own `applyLatencySpend(1)`; the two writes never observed each other, and global truth becomes `spent(1) + effectiveEncumbrance_A(100) = 101 > ceiling`. The race lives exactly in the zero-to-positive transition, and a decision made by reading encumbrance *outside* the coordination boundary can never account for a concurrent transition happening *inside* it.

**Corrected rule:** once the D1-A execution-coordination runtime exists and is in use — a property of the **runtime as deployed**, not a per-`DeliberationState` flag or any other new piece of persisted state (§28.34.3a) — **every** operation capable of increasing `deliberationState.latencyBudget.spent` for **any** `deliberationStateId` (every call site in §28.34.2's inventory, and any future one) must enter the same state-level coordination boundary (§28.34.4) **before**: reading `spent` for authorization purposes; reading `effectiveLatencyEncumbrance`; deciding whether it may proceed; or calling `applyLatencySpend`. This participation requirement is **unconditional with respect to the current encumbrance value** — the question "is effective encumbrance currently positive?" (§28.34.5's conservative policy) may be answered **only inside** the coordination boundary, never as a pre-boundary gate used to decide whether to enter it at all.

**28.34.3a Coordination-enabled vs. positive-encumbrance — two distinct concepts, not to be conflated:** "coordination-enabled" describes the **execution-coordination runtime's own operating mode** — once the D1-A runtime exists and new latency-consuming writes are routed through it, they are routed through it, full stop; it is not a per-`DeliberationState` toggle that could itself be read racily, and this document does not introduce one. "Positive effective encumbrance" is a single, ordinary accounting fact (§28.11) read fresh **inside** the coordination boundary on every entry — it decides only whether *this particular* write is admitted (§28.34.5), never whether coordination itself applies. Locking is never dynamically switched on or off based on positive encumbrance.

**28.34.4 State-level coordination critical section — now requires authoritative current-state access (fourth amendment, Blocker B); participant list corrected by the fifth amendment (Consistency Correction D):** freeze a `RouteExecutionCheckpointStore` capability conceptually:

```
withDeliberationStateCoordination(deliberationStateId, operation): result of operation
```

or an equivalent atomic/serialized mechanism, scoped per `deliberationStateId`, entered unconditionally (§28.34.3) by: (a) `claimRouteExecution`'s claim-and-reservation transaction (§28.7/§28.10); (b) `markExecutionOutcomeCommitted`'s current-`RouteOutcome` resolution and exact-agreement check (§28.19 — omitted from this list in the fourth amendment despite §28.17/§28.19 already requiring it; corrected here, no semantic change); (c) `persistExecutionTerminalFactAsRouteOutcome` (§28.35); (d) `recordExecutionPreCallFailure`'s full terminality/binding re-verification, not merely its live-policy check (§28.33, fifth amendment, Blocker A); (e) **every** new latency-consuming write, checkpointed or not (§28.34.2/§28.34.3).

**Serialization alone is insufficient (the demonstrated stale-snapshot problem):** this document's existing domain operations use immutable value semantics — `operation(state) → newState` (confirmed unchanged by direct inspection this packet: every `recordRouteOutcome`/`closeContextRequestWithoutResponse` branch returns a freshly-spread `{ ...state, ... }` object, never mutates its input). If two concurrent callers both capture the same `S0` before entering coordination, and B enters the critical section first (`S0 → S1`, `spent = 1`), then A entering the section *second* but still evaluating admission against its own **caller-captured `S0`** would see `spent = 0` and legally reserve 100 — the mutex serialized *execution order*, but did **not** make A observe `S1`. Serialization of *when* operations run is not the same guarantee as authoritative access to *current* state.

**Frozen dependency — `DeliberationStateAccessPort`:** a narrow orchestration/infrastructure dependency, responsible only for (i) resolving the **current** authoritative `DeliberationState` by id, from **inside** the state-level coordination boundary, and (ii) committing the new `DeliberationState` produced by a successful latency-consuming product operation **before** that boundary is released. It is explicitly **not** a second semantic ledger, not a replacement for `RouteOutcome`, not execution-checkpoint truth, and grants no provider/model authority (§28.34.8 restates this — no circularity). `RouteOutcome` remains product semantic truth (EC-25, unchanged).

**Frozen state-coordination transaction, conceptually:**

```
withCurrentDeliberationState(
  deliberationStateId,
  operation: (currentState) => { nextState?, result }
): result
```

with semantics: (1) acquire state coordination for `deliberationStateId`; (2) resolve the current authoritative `DeliberationState` **after** acquiring it (never before); (3) run `operation` against that current value; (4) if `operation` produces a new `DeliberationState`, commit it **before** releasing coordination; (5) release coordination; (6) return the result. No caller-supplied/caller-captured `DeliberationState` may be used as the authoritative accounting snapshot for a coordinated decision once the boundary is entered — a caller's own value may still be used to *identify* the intended state and to verify binding (§28.3), but never as the value admission math runs against. This applies uniformly to: claim admission; reservation calculation; commit-acknowledgement's current-`RouteOutcome` resolution (§28.19); checkpoint-backed `RouteOutcome` persistence (§28.35); pre-call terminality/binding verification (§28.33, fifth amendment); uncheckpointed `ADD_CONTEXT` latency writes; and any future `applyLatencySpend` writer. Storage/database mechanism is not chosen here; a first offline implementation may use an in-memory authoritative state cell, which — exactly like the store's own atomic-claim contract (§28.23) — demonstrates contract semantics only, never production cross-process durability.

**28.34.5 Conservative first-implementation policy (correctness over concurrency) — read only inside the boundary, per §28.34.3:** once inside the coordination boundary and reading the current authoritative state, if one or more checkpoints currently contribute positive active effective encumbrance for the `deliberationStateId`, an unrelated/uncheckpointed latency-consuming write is **not** permitted to bypass that reservation — the first runtime implementation MAY simply reject/defer such a write rather than attempt to share spare capacity concurrently, deliberately conservative, with no automatic retry implied. If, once inside the boundary, no positive active encumbrance is found, the canonical product-domain operation proceeds — but this determination is made *after* entering coordination and resolving current state, never as a pre-entry shortcut (§28.34.3's correction).

**Frozen wrapper for uncheckpointed writes, conceptually — checkpoint-exclusivity precondition added by the fifth amendment (Blocker B, see §28.36 for the full rule); closed-dispatch shape corrected by the D1-A Amendment 1 (runtime-discovered correction, below):**

**Runtime-discovered correction (D1-A Amendment 1, Blocker B):** implementing the shape below in its originally-frozen form — a caller-supplied `attemptId` plus a separately-supplied, arbitrary `operation: (currentState) => nextState` callback — exposed a concrete authority gap this document had not named: the `attemptId` checked against `RouteExecutionCheckpointStore` for the exclusivity gate and the `attemptId` the callback actually wrote a `RouteOutcome` for were two independently caller-selected facts that could disagree. Counterexample: attempt A already owns a `PreCallFailedTerminalFactReadyCheckpoint` (`reservedLatencyMs = 0`, `effectiveLatencyEncumbrance = 0`); attempt B owns no checkpoint. A caller supplies `attemptId = B` for the exclusivity check — which correctly finds no checkpoint and admits — while the callback it separately supplies calls `recordRouteOutcome({ attemptId: A, ... })`; the wrapper had no way to notice the callback wrote a different attempt than the one it just cleared, reopening exactly the EC-30/§28.36 direct-write bypass checkpoint-backed exclusivity exists to close. Corrected by removing the arbitrary-callback surface entirely and replacing it with a closed descriptor whose own `input.attemptId` is the single authority for both the exclusivity check and the write:

```
CoordinatedLatencyWriteInput =
  | { kind: 'ROUTE_OUTCOME'; input: RecordRouteOutcomeInput }
  | { kind: 'ADD_CONTEXT_NO_RESPONSE'; input: <exact closeContextRequestWithoutResponse input shape> }
...
The target `attemptId` is derived **exactly once**, from `write.input.attemptId` — never restated separately by the caller, and structurally incapable of resolving to two different values. Always enters state coordination first; resolves current state inside the boundary; derives the target `attemptId` from `write.input`; checks `RouteExecutionCheckpointStore` for an existing checkpoint on that **exact same identity** — if one exists, rejects outright, regardless of that checkpoint's `reservedLatencyMs`/`effectiveLatencyEncumbrance` (§28.36); if none exists, evaluates current `spent` + current `effectiveLatencyEncumbrance` there and dispatches to the ONE canonical product operation the descriptor names — `recordRouteOutcome` for `ROUTE_OUTCOME` (covering its `ADD_CONTEXT` `SUPPLIED`/`DECLINED`/generic-`FAILED` branches, and every other route's generic `FAILED`), or `closeContextRequestWithoutResponse` for `ADD_CONTEXT_NO_RESPONSE` — only if legally admissible; commits its returned state before releasing. The wrapper validates only its own closed descriptor shape (`kind`, `input`, and the single `attemptId` it derives) — it does not reimplement `RecordRouteOutcomeInput` or `ADD_CONTEXT`-closure semantic validation, which remain exclusively inside the two canonical operations it dispatches to, called unmodified (§28.34.8/EC-25). This covers every non-machine `ADD_CONTEXT` path without forcing `ADD_CONTEXT` into the provider-backed execution shell (§27.3/§28.4, unchanged) — and, per §28.36, the same checkpoint-existence check applies to any other coordinated attempt to persist a `RouteOutcome` directly, not only this specific wrapper. No new domain invariant is introduced by this correction — EC-30 already required exactly this outcome; the correction makes it actually enforceable at this call boundary, closing a gap between an already-frozen rule and a runtime shape that could not prove the identity it checked was the identity it wrote.

**28.34.6 No retroactive checkpointing:** this rule never retroactively checkpoints a historical, already-recorded outcome or closure — it governs only **new** writes attempted through the future coordination-capable orchestration layer (§28.21's legacy posture, unchanged and restated: historical no-checkpoint `RouteOutcome`s remain legitimate forever).

**28.34.7 Claim-vs-write race, corrected proof (fourth amendment) — both interleavings resolve against current, never stale, state:**

- **Case A — claim wins the state coordination section:** current state is read inside the boundary; the reservation is committed; the unrelated latency-consuming write, entering later, itself enters coordination, re-reads the now-current state and its resulting reservation/encumbrance, and cannot bypass it.
- **Case B — the latency-consuming write wins the section:** current state is read inside the boundary; the product write returns a new state; that new state is committed (`DeliberationStateAccessPort`) **before** the boundary releases; the claim, entering later, itself enters coordination and re-reads **that already-committed** authoritative state — its admission check sees the new `spent` value, never the pre-write one.

No stale `S0` survives into the second operation in either case, because neither operation is ever permitted to evaluate admission against a value captured before it entered the boundary.

**28.34.8 No domain semantic circularity:** `DeliberationStateAccessPort` exists only to provide current-value authority and atomic state-update ordering — it does not make checkpoint state semantically authoritative. `recordRouteOutcome` and `closeContextRequestWithoutResponse` remain the canonical, unmodified product-domain validators (§28.27/EC-25); execution coordination controls *admission and ordering* around them, never their semantic content.

### 28.35 Checkpoint-backed `RouteOutcome` persistence & exact terminal-fact/outcome agreement (Slice 2D-D1-A0 third amendment, Blockers G and E; sequence corrected by the fourth amendment, Blocker B)

**28.35.1 Why replay, not re-entry, is required:** two under-specifications converge here — (A) a checkpoint's `actualLatencyMs` (an integer, independently validated at terminal-fact recording, §28.14) could otherwise be re-typed by a caller as a different, merely "close enough" legal number when later calling `recordRouteOutcome`; (B) a checkpoint's `FailureInfo`/route-specific result payload could otherwise be re-typed as a semantically similar but unequal value. For any checkpoint-backed write, the terminal fact is captured exactly once, at `recordExecutionTerminalFact` (or `recordExecutionPreCallFailure`) time — product persistence must **replay** it, never accept a second, independently-authored re-entry of the same result.

**28.35.2 Frozen coordination-layer operation:**
...
**`deliberationState`'s role is narrowed by the fourth amendment:** the caller-supplied `deliberationState` may be used only to *identify* the intended state and to *verify* the caller's own binding expectations (§28.3) — it is **never** the value admission/persistence math runs against once coordination is entered (§28.34.4/§28.34.7). The authoritative accounting snapshot is always re-resolved inside the boundary, per the corrected sequence below.

**Frozen sequence (fourth amendment — corrected to close the stale-snapshot gap, §28.34.4):** (1) resolve the checkpoint for `attemptId` from `store` (an identity lookup by the checkpoint's own key, §28.1 — not a racy accounting read; the checkpoint's `deliberationStateId`/`route` binding is fixed once set, §28.4, and is not itself subject to concurrent mutation); (2) using that `deliberationStateId`, acquire state-level coordination (§28.34.4); (3) resolve the **current** authoritative `DeliberationState` **inside** that boundary via `DeliberationStateAccessPort` — never the caller-supplied `deliberationState` value; (4) re-verify the checkpoint is still `TERMINAL_FACT_READY` against this current state (defends against a concurrent second commit attempt) and reject outright if `terminalFact.kind === 'DEADLINE_VIOLATION'` (§28.14 — never committable); (5) derive the exact `RecordRouteOutcomeInput` from the **stored** terminal fact — `status` from `terminalFact.kind` (§28.13's closed mapping: `SUCCEEDED → SUCCEEDED`, `INCONCLUSIVE → INCONCLUSIVE`, `FAILED → FAILED`), `latencyConsumed` from `terminalFact.actualLatencyMs`, and the route-specific fields spread in directly from `terminalFact.payload` or `terminalFact.failure` — a purely mechanical reassembly, since `RouteSpecificSuccessPayload`/`SeekEvidenceInconclusivePayload` were already frozen (§28.13) as exact subsets of `RecordRouteOutcomeInput`, never a new normalization step; (6) call the canonical, unmodified `recordRouteOutcome` against the **current** state resolved in step 3, with `session` re-verified for binding as always (§28.3); (7) never call a provider at any point; (8) commit the resulting new `DeliberationState` through `DeliberationStateAccessPort` **before** releasing the state coordination boundary; (9) release coordination and return the new `DeliberationState`; (10) if the product write fails or rejects, commit nothing, leave the checkpoint at `TERMINAL_FACT_READY` (§27.11's "product persistence failure" posture, unchanged) — the same checkpointed snapshot remains available for a later persistence-only reconciliation attempt (§27.11), never a second call to the coordination-layer's own terminal-fact-recording step. A caller supplies `{ attemptId }` only (§28.18) — never `status`, `latencyConsumed`, `failure`, `result`, `citations`, `targetRef`/`sourceRef`, `response`, `findingIds`, `boundedExcerpt`, or any other terminal-payload field; all are derived exclusively from the stored terminal fact (EC-23) and the freshly-resolved current state (EC-27).

**28.35.3 `projectRouteOutcomeToTerminalFact` — the shared comparison boundary:**

```
projectRouteOutcomeToTerminalFact(outcome: RouteOutcome): { kind, actualLatencyMs, payload | failure }
```

A pure, symmetric projection of an **existing, authoritative** `RouteOutcome` back into the same `ReservedTerminalFact`-shaped structure a checkpoint stores — reconstructing `kind` (`SUCCEEDED`/`INCONCLUSIVE`/`FAILED`, from `outcome.status`), `actualLatencyMs` (from `outcome.latencyConsumed`), and the exact route-specific payload or `failure`. **Checkpoint agreement means value equality of these closed domain fields** — never object identity, never "same category only," never prose similarity, never a first/latest heuristic. Where a domain field is an ordered array with domain-significant ordering (e.g. `citations`, `findingIds`), ordering is preserved and compared, not merely set-membership. No generic unbounded JSON equality over provider payloads is needed or introduced, because raw provider payloads are already structurally excluded from every field this comparison touches (§28.13, §26.10). This projection is the exact mechanism `markExecutionOutcomeCommitted` (§28.19 step 3) and `assertRouteExecutionCheckpointIntegrity` (§28.20) both use, and the exact mechanism §28.11's residual-encumbrance match now requires (tightened by this amendment) — a single shared boundary, not three independently-maintained comparison implementations.

**28.35.4 No circular authority preserved:** `recordRouteOutcome`'s own validators are entirely unaware of, and never inspect, checkpoint content (§28.27/EC-25, unchanged) — `persistExecutionTerminalFactAsRouteOutcome` is a caller of `recordRouteOutcome`, not a modification to it, and `projectRouteOutcomeToTerminalFact` reads an already-persisted `RouteOutcome`'s own fields, never the reverse.

### 28.36 Checkpoint-backed `RouteOutcome` exclusivity — no direct-write bypass (Slice 2D-D1-A0 fifth amendment, Blocker B)

**28.36.1 The bypass, demonstrated:** a `PreCallFailedTerminalFactReadyCheckpoint` legitimately carries `reservedLatencyMs = 0` and `effectiveLatencyEncumbrance = 0` (§28.11) — by design, since no provider call was ever attempted for it. Budget/encumbrance coordination alone (§28.34) therefore cannot prevent a second, unrelated write from targeting the *same* `attemptId`. Counterexample: attempt A has a checkpoint with `terminalFact = { kind: 'FAILED', actualLatencyMs: 0, failure: { category: 'REFERENCE_RESOLUTION', message: 'X' } }`. A caller bypasses `persistExecutionTerminalFactAsRouteOutcome` and instead attempts a *direct*, coordinated call to the canonical `recordRouteOutcome({ attemptId: A, status: 'FAILED', latencyConsumed: 0, failure: { category: 'EXECUTION', message: 'Y' } })`. The canonical validator, correctly and by design (§28.27/EC-25), does not inspect checkpoint state at all — it sees no prior `RouteOutcome` for A, `latencyConsumed = 0` is legal, and a generic `FAILED` is legal for any non-`STOP` route — so the write succeeds, landing a `RouteOutcome` that **disagrees** with the checkpointed fact (`EXECUTION`/`Y` vs. the stored `REFERENCE_RESOLUTION`/`X`). A later `markExecutionOutcomeCommitted` rejection (§28.19's exact-agreement check) is too late — the disagreeing, caller-retyped `RouteOutcome` has already been persisted as product truth.

**28.36.2 Frozen rule — checkpoint-backed exclusivity, an identity/authority gate, not a budget gate:** for any **new** `RouteOutcome` write in the coordination-enabled runtime, if an execution checkpoint already exists for that `attemptId`, the **only** legal product-terminal persistence path is `persistExecutionTerminalFactAsRouteOutcome` (§28.35). No generic or uncheckpointed write wrapper may call `recordRouteOutcome` directly for an `attemptId` that already owns any execution checkpoint — **regardless of checkpoint phase, `reservedLatencyMs`, `effectiveLatencyEncumbrance`, or terminal-fact kind.** This is gated on the checkpoint's mere *existence* for the target `attemptId`, never on its current budget occupancy.

**28.36.3 Zero reservation does not mean uncheckpointed:** `effectiveLatencyEncumbrance === 0` does **not** imply "this attempt has no execution coordination" — a `PreCallFailedTerminalFactReadyCheckpoint`, and any `ReservedTerminalFactReadyCheckpoint` whose matching outcome has already landed (§28.11's residual-to-zero case), are both still checkpoint-backed execution history. Budget occupancy and execution provenance are separate questions; only the latter governs this exclusivity rule.

**28.36.4 Coordinated product-write dispatch, conceptually:** when any operation intends to persist a new `RouteOutcome`: (1) enter state coordination for the target `deliberationStateId`; (2) resolve the current authoritative `DeliberationState` (§28.34.4); (3) resolve the target `attemptId`; (4) inspect `RouteExecutionCheckpointStore` for an existing checkpoint on that `attemptId`. If a checkpoint exists, the direct canonical write is rejected — the caller must use `persistExecutionTerminalFactAsRouteOutcome` instead. If no checkpoint exists, the legacy/offline/uncheckpointed canonical `RouteOutcome` path remains fully legal, subject to the ordinary product validators (§28.27/EC-25, unchanged) and the latency-write coordination boundary (§28.34) — preserving historical compatibility (§28.21, unchanged) without retroactively synthesizing a checkpoint for anything.

**28.36.5 `ADD_CONTEXT` interaction:** `ADD_CONTEXT` remains categorically non-machine (§27.3/§28.4) and normally has no execution checkpoint at all. Its `SUPPLIED`/`DECLINED`/generic-`FAILED`/`NO_RESPONSE` latency-consuming writes still enter state coordination (§28.34, unchanged) via `performCoordinatedLatencyWrite` (§28.34.5), which now also carries this section's checkpoint-existence check (§28.34.5, fifth amendment). If a checkpoint somehow exists for the same `ADD_CONTEXT` attempt — an anomalous but not impossible state, since nothing in this document forbids constructing one for a non-machine route at the schema level — that is not a reason to silently ignore it: the direct write rejects/fails closed exactly as it would for any other route, rather than bypassing checkpoint provenance. This introduces no provider-backed execution for `ADD_CONTEXT`.

**28.36.6 Generic `FAILED`'s `MIXED` classification does not decide this gate:** the fourth amendment already established that `recordRouteOutcome`'s generic `FAILED` call site (§28.34.2, row 1) is `MIXED` — it terminates any non-`STOP` route, machine or `ADD_CONTEXT` alike — so runtime dispatch can never decide checkpoint-backed-vs-uncheckpointed from *which source branch* is about to run. §28.36.4's dispatch decides it correctly: from the actual attempt/checkpoint identity (does a checkpoint exist for this exact `attemptId`?), never from the static route or branch shape of the intended write.

**28.36.7 EC-30 grounding, restated:** this section is the concrete mechanism behind EC-30 (§28.26) — once a checkpoint exists for an `attemptId`, every subsequent new `RouteOutcome` for that attempt is checkpoint-backed and may be persisted only through `persistExecutionTerminalFactAsRouteOutcome`'s exact stored-terminal-fact replay (§28.35.1–§28.35.3); zero reservation and zero effective encumbrance never restore an uncheckpointed write path.
